...
def test_evaluate_progression_single_sample_vs_stable() -> None:
    ...
    res_single = evaluate_progression(
        previous_evidence=prev_ev, current_evidence=curr_ev_single, skill="writing"
    )
    ...
    res_stable = evaluate_progression(
        previous_evidence=prev_ev, current_evidence=curr_ev_repeated, skill="writing"
    )
...
def test_non_comparable_results_cannot_create_stable_progression() -> None:
    ...
    current = (
        ...
        },
        {
            ...
            "skill": "writing",
            "comparable": False,
            "comparison_key": "translation",
        },
    )

    result = evaluate_progression(
        previous_evidence=previous, current_evidence=current, skill="writing"
    )
...
def test_progress_operation_never_inflates_unrelated_skills() -> None:
    ...
    assert set(result["skill_progress"]) <= {
        item["skill"] for item in ({"skill": "writing"},)
    }
...
def test_evaluate_progression_one_poor_session_no_stable_regression() -> None:
    ...
    res = evaluate_progression(
        previous_evidence=prev_ev, current_evidence=curr_ev_bad, skill="writing"
    )
...
def test_evaluate_certification_source_authority() -> None:
    ...
    conflicting = (
        ...
        },
        {
            ...
            "date_valid": True,
            "task_count": 4,
            "official_source_id": "off-2",
        },
    )
    res_conf = evaluate_certification_source(
        sources=conflicting, decision_critical=True
    )
...
def test_evaluate_language_memory_consent() -> None:
    ...
    res_session = evaluate_language_memory_consent(
        content_kind="session_note", session_only=True
    )
...
def test_remaining_rule_classes_evaluation() -> None:
    ...
    # 1. SpacedReviewRule
    rule_sr = SpacedReviewRule(
        definition=DomainReasoningRuleDefinition(
            id="languages.spaced_review",
            name="SpacedReviewRule",
            version="1.0.0",
    ...
    res_sr = rule_sr.evaluate(
        ReasoningRuleContext(
            reasoning_id="r-sr",
            timestamp=NOW,
            metadata={"material": {"items": [{"id": "v1", "due": True}]}},
        )
    )
    assert res_sr.status == ReasoningRuleResultStatus.APPLIED

    # 2. LearningLoadRule
    rule_ll = LearningLoadRule(
        definition=DomainReasoningRuleDefinition(
            id="languages.learning_load",
            name="LearningLoadRule",
            version="1.0.0",
            scope=ReasoningRuleScope.DOMAIN,
            domain_id="domain:languages",
            category=ReasoningRuleCategory.INFERENCE,
    ...
    res_ll = rule_ll.evaluate(
        ReasoningRuleContext(
            reasoning_id="r-ll",
            timestamp=NOW,
            metadata={"material": {"available_time": 25}},
        )
    )
    assert res_ll.status == ReasoningRuleResultStatus.APPLIED

    # 3. GoalAlignmentRule
    rule_ga = GoalAlignmentRule(
        definition=DomainReasoningRuleDefinition(
            id="languages.goal_alignment",
            name="GoalAlignmentRule",
            version="1.0.0",
            scope=ReasoningRuleScope.DOMAIN,
            domain_id="domain:languages",
            category=ReasoningRuleCategory.INFERENCE,
    ...
    res_ga = rule_ga.evaluate(
        ReasoningRuleContext(
            reasoning_id="r-ga",
            timestamp=NOW,
            metadata={"material": {"goals": [{"id": "g1"}]}},
        )
    )
    assert res_ga.status == ReasoningRuleResultStatus.APPLIED

    # 4. ProgressionEvidenceRule
    rule_pe = ProgressionEvidenceRule(
        definition=DomainReasoningRuleDefinition(
            id="languages.progression_evidence",
            name="ProgressionEvidenceRule",
            version="1.0.0",
            scope=ReasoningRuleScope.DOMAIN,
            domain_id="domain:languages",
    ...
    res_pe = rule_pe.evaluate(
        ReasoningRuleContext(
            reasoning_id="r-pe",
            timestamp=NOW,
            metadata={"material": {"current_evidence": [{"score": 0.8}]}},
        )
    )
    assert res_pe.status == ReasoningRuleResultStatus.APPLIED

    # 5. CertificationTemporalRule
    rule_ct = CertificationTemporalRule(
        definition=DomainReasoningRuleDefinition(
            id="languages.certification_temporal",
            name="CertificationTemporalRule",
            version="1.0.0",
            scope=ReasoningRuleScope.DOMAIN,
            domain_id="domain:languages",
            category=ReasoningRuleCategory.TEMPORALITY,
            status=ReasoningRuleStatus.ENABLED,
            priority=810,
            risk_level=ReasoningRiskLevel.LOW,
        )
    )
    res_ct = rule_ct.evaluate(
        ReasoningRuleContext(
            reasoning_id="r-ct",
            timestamp=NOW,
            metadata={"material": {"sources": [{"source_type": "official"}]}},
        )
    )
    assert res_ct.status == ReasoningRuleResultStatus.APPLIED

    # 6. CulturalContextEvidenceRule
    rule_cc = CulturalContextEvidenceRule(
        definition=DomainReasoningRuleDefinition(
            id="languages.cultural_context_evidence",
            name="CulturalContextEvidenceRule",
            version="1.0.0",
            scope=ReasoningRuleScope.DOMAIN,
            domain_id="domain:languages",
            category=ReasoningRuleCategory.EPISTEMIC,
    ...
    res_cc = rule_cc.evaluate(
        ReasoningRuleContext(
            reasoning_id="r-cc",
            timestamp=NOW,
            metadata={"material": {"claim": "Tendency in region"}},
        )
    )
    assert res_cc.status == ReasoningRuleResultStatus.APPLIED

    # 7. LanguageMemoryConsentRule
    rule_mc = LanguageMemoryConsentRule(
        definition=DomainReasoningRuleDefinition(
            id="languages.language_memory_consent",
            name="LanguageMemoryConsentRule",
            version="1.0.0",
            scope=ReasoningRuleScope.DOMAIN,
            domain_id="domain:languages",
            category=ReasoningRuleCategory.SAFETY,
            status=ReasoningRuleStatus.ENABLED,
            priority=830,
            risk_level=ReasoningRiskLevel.LOW,
        )
    )
    res_mc = rule_mc.evaluate(
        ReasoningRuleContext(
            reasoning_id="r-mc",
            timestamp=NOW,
            metadata={"material": {"session_only": True}},
        )
    )
...
def test_progression_general_scope_rejects_skill_specific_evidence() -> None:
    """Progression evaluated for general scope rejects skill-specific observations."""
    prev = (
        {
            "provenance_id": "p1",
            "skill": "writing",
    ...
    res_none = evaluate_progression(
        previous_evidence=prev, current_evidence=curr, skill=None
    )
    assert res_none["stable_progression"] is False
    assert res_none["progression_outcome"] == "insufficient_evidence"
    assert res_none["skill"] == "general"

    res_gen = evaluate_progression(
        previous_evidence=prev, current_evidence=curr, skill="general"
    )
    assert res_gen["stable_progression"] is False
    assert res_gen["progression_outcome"] == "insufficient_evidence"
    assert res_gen["skill"] == "general"


def test_progression_unknown_skill_fails_closed() -> None:
    """Progression evaluated with unknown skill fails closed."""
    prev = (
        {
            "provenance_id": "p1",
            "skill": "foobar",
    ...
    res = evaluate_progression(
        previous_evidence=prev, current_evidence=curr, skill="foobar"
    )
    assert res["stable_progression"] is False
    assert res["progression_outcome"] == "insufficient_evidence"
    assert res["skill"] == "foobar"
