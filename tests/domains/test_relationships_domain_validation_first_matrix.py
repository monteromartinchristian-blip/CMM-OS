"""Phase 10.21 — Relationships Domain validation-first adversarial matrix.

Proves that ``register_relationships_domain`` is genuinely **validation-first**:
...
The matrix covers all 13 validation paths that can reject a Relationships
Domain registration, plus the permission prevalidation guarantee that an
unrelated error is never broadly swallowed as "already registered".
"""
...
class _CountingDomainRegistry(DomainRegistry):
    ...
    This is the *first* registry mutated by ``register_relationships_domain``,
    so a ``register_calls == 0`` result is a direct proof of validation-first.
    """
...
def _baseline(registries):
    ...
    return {
        ...
        "workflow": len(
            registries["workflow_registry"].list_for_domain(RELATIONSHIPS_DOMAIN_ID)
        ),
...
def _assert_unchanged(registries, counting, baseline):
    ...
    assert (
        len(registries["workflow_registry"].list_for_domain(RELATIONSHIPS_DOMAIN_ID))
        == baseline["workflow"]
    )
    assert (
...
def test_2_duplicate_profile_id_rejected_before_mutation():
    registries, counting = _spy_registries()
    registries["profile_registry"].register(build_relationships_profile())
...
def test_3_existing_profile_for_relationships_domain_rejected_before_mutation():
    """A profile already present for the relationships domain (different id) must
    ...
    registries["profile_registry"].register(
        DomainProfileDefinition(
            id="other.relationships.profile",
            domain_id=RELATIONSHIPS_DOMAIN_ID,
            profile_name="OtherRelationships",
        )
    )
...
def test_4_duplicate_resource_rejected_before_mutation():
    ...
    registries["resource_registry"].register(
        build_relationships_resource_definitions()[0]
    )
...
def test_5_duplicate_rule_rejected_before_mutation():
    ...
    registries, counting = _spy_registries()
    registries["rule_registry"].register(build_relationships_rules()[0])
...
def test_7_mismatched_operation_implementation_rejected_before_mutation():
    ...
    registries, counting = _spy_registries()
    ops = build_relationships_operation_definitions()
...
def test_8_invalid_operation_execute_signature_rejected_before_mutation():
    ...
    registries, counting = _spy_registries()
    declared = build_relationships_operation_definitions()[0]
...
def test_9_duplicate_relationships_operation_rejected_before_mutation():
    registries, counting = _spy_registries()
    op_registry = registries["operation_registry"]
    op_registry.register(build_relationships_operation_definitions()[0])
...
def test_10_common_operation_collision_rejected_before_mutation():
    ...
    registries, counting = _spy_registries()
    op_registry = registries["operation_registry"]
    operation = build_relationships_operation_definitions()[0]
...
def test_11_duplicate_relationships_workflow_rejected_before_mutation():
    registries, counting = _spy_registries()
    registries["workflow_registry"].register(
        build_relationships_workflow_definitions()[0]
    )
...
def test_12_common_workflow_collision_rejected_before_mutation():
    ...
    workflow = build_relationships_workflow_definitions()[0]

    workflow_registry.common_registry.register(workflow.to_common())
    assert workflow_registry.list_for_domain(RELATIONSHIPS_DOMAIN_ID) == ()
...
def test_13_pre_registered_permission_policy_rejected_before_mutation():
    registries, counting = _spy_registries()
    registries["permission_registry"].register(build_relationships_permission_policy())
...
def test_permission_prevalidation_only_swallows_not_found():
    ...
    registries, counting = _spy_registries()
    result = register_relationships_domain(
        **registries, operation_implementations=_implementations()
    )
    assert str(result.definition.id) == RELATIONSHIPS_DOMAIN_ID
    assert counting.register_calls == 1
    assert (
        registries["permission_registry"].get("domain-permission:relationships:1.0.0")
        is not None
    )
