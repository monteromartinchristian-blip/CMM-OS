...
def test_global_deny_and_domain_allow_resolve_to_deny():
    ...
    registry.register(
        DomainPermissionPolicy("p", "domain:health", "1.0.0", allow_memory_read=True)
    )
    resolver = DomainPermissionResolver(registry)
    request = DomainPermissionRequest(
        "r", PermissionCapability.MEMORY_READ, "domain:health", "a", "s"
    )
    result = resolver.resolve(
        request,
        layer_evaluations=(
            PermissionLayerEvaluation(
                PermissionLayer.GLOBAL, PermissionOutcome.DENY, reasons=("global",)
            ),
        ),
    )
...
def test_supporting_domain_deny_cannot_be_hidden_by_primary_allow():
    ...
    registry.register(
        DomainPermissionPolicy(
            "primary", "domain:life", "1.0.0", allow_memory_read=True
        )
    )
    registry.register(
        DomainPermissionPolicy(
            "support", "domain:health", "1.0.0", allow_memory_read=False
        )
    )
    resolver = DomainPermissionResolver(registry)
    request = DomainPermissionRequest(
...
def test_cross_domain_enforces_declared_duration_and_scope_constraints():
    ...
    request = CrossDomainPermissionRequest(
        ...
        session_id="s",
        sensitivity_level="internal",
        duration=CrossDomainDuration.SESSION,
        constraints={
...
def test_resolution_generates_structured_conflict_for_primary_allow_supporting_deny():
    ...
    result = DomainPermissionResolver(registry).resolve(
        request, supporting_domains=("domain:health",)
    )
...
def test_legacy_approval_grants_never_authorize_resolution():
    ...
    resolver = DomainPermissionResolver(registry)
    request = DomainPermissionRequest(
        "r", PermissionCapability.MEDICAL_DECISION, "domain:health", "actor", "session"
    )
    now = datetime(2026, 8, 2, tzinfo=timezone.utc)
    pending = resolver.resolve(request, now=now)
    requirement = pending.approval_requirements[0]
    bad_grant = PermissionApprovalGrant(
        replace(requirement, actor_id="other"), "2026-08-02T00:00:00+00:00"
    )
    assert (
        resolver.resolve(
            request, approval_grants=(bad_grant,), now=now
...
def test_cross_domain_temporal_policy_requires_clock_and_uses_it_for_both_sides():
    ...
    assert (
        resolver.resolve_cross_domain(
            request, now=datetime(2026, 8, 2, tzinfo=timezone.utc)
        ).decision
        is PermissionOutcome.APPROVAL_REQUIRED
    )
    assert (
        resolver.resolve_cross_domain(
            request, now=datetime(2026, 8, 4, tzinfo=timezone.utc)
        ).decision
