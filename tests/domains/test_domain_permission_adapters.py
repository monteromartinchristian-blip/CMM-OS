...
def test_operation_adapter_uses_real_definition_and_requires_approval_for_destructive():
    ...
    operation = DomainOperationDefinition(
        ...
        version="1.0.0",
        name="Delete",
        description="Delete a reversible record",
...
def test_operation_adapter_evaluates_required_permission_with_provenance():
    ...
    assert (
        decision.requirement_decisions[0].permission
...
def test_operation_adapter_requires_approval_for_irreversible_and_external_capability():
    ...
    assert decision.decision is PermissionOutcome.APPROVAL_REQUIRED
    assert {item.permission for item in decision.requirement_decisions} == {
        PermissionCapability.SEARCH_EXTERNAL.value
    }
    assert {item.reason_code for item in decision.approval_requirements} >= {
...
def test_operation_adapter_denies_disabled_and_missing_external_capability():
    ...
    external = DomainOperationDefinition(
        "project.external",
        "domain:project",
        "1.0.0",
        "External",
        "External",
        DomainOperationType.EXTERNAL,
        reversible=True,
    )
    disabled = DomainOperationDefinition(
        "project.read",
        "domain:project",
        "1.0.0",
        "Read",
        "Read",
        DomainOperationType.READ,
        reversible=True,
        enabled=False,
    )
    ...
    assert (
        evaluate_domain_operation(
            external, resolver, request_id="r", actor_id="a", session_id="s"
        ).decision
        is PermissionOutcome.DENY
    )
    assert (
        evaluate_domain_operation(
            disabled, resolver, request_id="r", actor_id="a", session_id="s"
        ).decision
        is PermissionOutcome.DENY
    )
...
def test_operation_adapter_propagates_approval_for_a_required_permission():
    ...
    operation = DomainOperationDefinition(
        ...
        reversible=True,
        required_permissions=(PermissionCapability.MEDICAL_DECISION.value,),
    )
    decision = evaluate_domain_operation(
        operation,
        DomainPermissionResolver(registry),
    ...
    assert decision.decision is PermissionOutcome.APPROVAL_REQUIRED
    assert (
        decision.requirement_decisions[0].decision
...
def test_workflow_adapter_evaluates_each_node_and_preserves_optional_blocks():
    execute = WorkflowNode(
        ...
        WorkflowNodeType.EXECUTE_OPERATION,
        "Execute",
        operation_id="project.read",
    )
    optional = WorkflowNode(
        "optional",
        WorkflowNodeType.EXECUTE_OPERATION,
        "Optional",
        operation_id="project.missing",
        required=False,
    )
    gate = WorkflowNode(
        "gate", WorkflowNodeType.REQUEST_APPROVAL, "Gate", approval_gate="review"
    )
    workflow = DomainWorkflowDefinition(
        "project.workflow",
        "domain:project",
    ...
    registry.register(
        DomainPermissionPolicy(
            ...
            allowed_operations=("project.read",),
        )
    )
    decision = evaluate_domain_workflow(
        workflow,
    ...
    assert {node.node_id for node in decision.node_decisions} == {
        "execute",
        "optional",
        "gate",
    }
...
def test_workflow_adapter_evaluates_required_resource_id_without_treating_it_as_kind():
    ...
    assert (
        "resource_allowlist_not_matched"
        not in decision.requirement_decisions[0].reasons
    )
    assert (
        "resource_kind_allowlist_not_matched"
        not in decision.requirement_decisions[0].reasons
    )
...
def test_workflow_adapter_denies_cross_domain_subworkflow_node():
    ...
    child = DomainWorkflowDefinition(
        "health.workflow", "domain:health", "1.0.0", "Child", nodes=(child_node,)
    )
    registry = DomainPermissionRegistry()
    registry.register(
        DomainPermissionPolicy(
...
def test_rule_adapter_allows_partial_enforcement_for_optional_rule():
    rule = DomainReasoningRuleDefinition(
        ...
        domain_id="domain:project",
        required_permissions=("permission.modify",),
    )
    registry = DomainPermissionRegistry()
    registry.register(DomainPermissionPolicy("project", "domain:project", "1.0.0"))
    decision = evaluate_domain_rule(
        rule,
        DomainPermissionResolver(registry),
        request_id="r",
        actor_id="a",
