...
def test_required_approval_is_waiting_until_gate_is_approved():
    definition = DomainWorkflowDefinition(
        ...
        nodes=(
            WorkflowNode(
                "approve", "request_approval", "Approve", approval_gate="gate-1"
            ),
        ),
    )
    ...
    available = resolve_domain_workflow(
        definition,
        DomainWorkflowContext("domain:x", approved_gates=frozenset({"gate-1"})),
    )
...
def test_domain_executor_returns_validated_domain_result_after_wait_resume():
    ...
    definition = DomainWorkflowDefinition(
        ...
        nodes=(
            WorkflowNode(
                "ask", "ask_question", "Ask", wait_condition={"field": "answer"}
            ),
    ...
    def operation_adapter(node, run):
        ...
        if node.node_id == "ask" and "answer" not in run.inputs:
            return NodeExecution.wait(
                WaitRequest("input", "answer required", "ask", {"field": "answer"})
            )
        return NodeExecution.complete({"ok": True})

    executor = DomainWorkflowExecutor(
        id_factory=lambda: "id", operation_adapter=operation_adapter
    )
...
def test_domain_approval_node_resumes_only_with_its_bound_approval():
    definition = DomainWorkflowDefinition(
        ...
        nodes=(
            WorkflowNode(
                "approve", "request_approval", "Approve", approval_gate="gate"
            ),
        ),
    )
    executor = DomainWorkflowExecutor(id_factory=lambda: "id")
    waiting = executor.execute(
        definition, DomainWorkflowContext("domain:x"), {"amount": 3}
    )
    assert waiting.status is WorkflowRunStatus.WAITING_FOR_APPROVAL
    approval = waiting.common_run.wait_request.approval_request.decide(
