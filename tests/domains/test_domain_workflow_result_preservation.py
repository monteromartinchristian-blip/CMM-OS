...
def test_execute_result_preserves_operation_result_events_and_attempts():
    ...
    def adapter(node, run):
        return (
            ...
            else NodeExecution.complete({"done": True})
        )

    executor = DomainWorkflowExecutor(
        id_factory=lambda: "id", operation_adapter=adapter
    )
    result = executor.execute_result(
        definition,
        DomainWorkflowContext("domain:x", available_operations=frozenset({"op.x"})),
        {},
    )
    assert result.status is WorkflowRunStatus.COMPLETED
    assert (
        result.common_result.node_results["op"].status is WorkflowNodeStatus.COMPLETED
    )
...
def test_resume_rehydrates_results_events_attempts_outputs_and_hierarchy():
    definition = DomainWorkflowDefinition(
        ...
        nodes=(
            ...
            WorkflowNode(
                "b", "request_approval", "B", dependencies=("a",), approval_gate="gate"
            ),
    ...
    def adapter(node, run):
        ...
        if node.node_id == "a":
            return NodeExecution.complete(
                {"a": True}, operation_result={"operation": "a"}
            )
    ...
    first = executor.execute(
        definition,
        DomainWorkflowContext("domain:x", available_operations=frozenset({"op.a"})),
        {"request": 1},
    )
    approval = first.common_run.wait_request.approval_request.decide(
        ApprovalDecision("human", True)
    )
    ...
    assert resumed.common_run.outputs == {
        "a": {"a": True},
        "b": {"b": True},
        "c": {"c": True},
    }
