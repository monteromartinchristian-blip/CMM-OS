"""Tests for Phase 10.21 Relationships Domain memory integration
(proposal-only)."""
...
def _reference(reference_id: str, canonical_id: str) -> DomainMemoryReference:
    return DomainMemoryReference(
        ...
        applicable_domains=("domain:relationships",),
        evidence_ids=("ev:1",),
        resource_ids=("res:1",),
    )
...
def test_view_request_domain_is_relationships():
    request = build_relationships_memory_view_request(request_id="req1")
    assert str(request.primary_domain) == "domain:relationships"


def test_memory_proposal_requires_confirmation():
    proposal = build_relationships_memory_proposal(proposal_id="prop1")
...
def test_proposal_confirmation_invariant_not_overridable():
    """Sensitive Relationships memory must always require confirmation.

    A caller must not be able to opt out of confirmation for a memory
...
def test_memory_view_resolved():
    ...
    request = build_relationships_memory_view_request(
        request_id="req1",
        requested_kinds=(DomainMemoryReferenceKind.KNOWLEDGE_ITEM,),
        candidates=(ref,),
    )
    view = build_relationships_memory_view(request=request, inventory=_inventory(ref))
    assert view.primary_domain.slug == "relationships"
...
def test_binding_validates_against_inventory():
    ...
    inventory = DomainMemoryReferenceInventory(
        ...
        permission_decisions=(
            DomainMemoryPermissionDecisionSnapshot(
                ...
                source_domain_id="domain:relationships",
                target_domain_id="domain:relationships",
            ),
        ),
        approval_requests=(
            DomainMemoryApprovalRequestSnapshot(
        ...
        traces=(
            DomainMemoryTraceSnapshot(
                trace_id="trace1", primary_domain="domain:relationships"
            ),
        ),
        views=(
            DomainMemoryViewSnapshot(
    ...
    result = validate_relationships_memory_binding(binding=binding, inventory=inventory)
