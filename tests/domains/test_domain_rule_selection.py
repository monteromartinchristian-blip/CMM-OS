    ...
    def __init__(
        ...
        *,
        ...
        self.definition = ReasoningRuleDefinition(
            ...
            version="1.0.0",
            scope="domain" if domain else "global",
            domain_id=domain,
...
def profile(
    *,
    ...
    resolver = DefaultDomainProfileResolver(
        ...
        profile_id_factory=lambda: "resolved-profile-1",
    ...
    return resolver.resolve(
        ...
        global_profile=DomainProfileDefinition(
            id="global", domain_id=DomainId("general"), profile_name="GeneralProfile"
        ),
        ...
        supporting_profiles=tuple(
            DomainProfileDefinition(
                id=f"support-{item}",
                domain_id=DomainId(item),
                profile_name=f"{item.title()}Profile",
            )
...
def test_required_optional_prohibited_and_global_order() -> None:
    ...
    for rule in (
        Rule("global.base", priority=1),
        Rule("security.guard", priority=1),
...
def test_optional_prohibited_is_omitted_and_missing_optional_is_partial() -> None:
    ...
    plan = selector().select(
        registry=registry,
        profile=profile(
            optional=("health.optional", "health.missing"),
            prohibited=("health.optional",),
        ),
    )
...
def test_required_missing_disabled_or_permission_denied_blocks() -> None:
    for registry in (
        ...
        _registry(
            Rule(
                "health.required", domain="domain:health", permissions=("health.read",)
            )
        ),
    ):
        plan = selector().select(
            registry=registry,
            profile=profile(required=("health.required",)),
...
def test_wrong_domain_blocks_required_and_primary_precedes_supporting() -> None:
    ...
    resolved = profile(
        required=("health.rule", "project.rule"), supporting=("project",)
    )
    plan = selector().select(
        registry=_registry(
...
def test_deduplication_preserves_multiple_sources_and_exact_version() -> None:
    ...
    plan = selector().select(
        registry=registry,
        profile=profile(optional=("global.base@1.0.0",)),
        global_mandatory_rules=("global.base",),
        requested_rule_ids=("global.base",),
    )
...
def test_required_prohibited_from_composition_blocks_and_global_is_preserved() -> None:
    registry = _registry(
        Rule("global.base"), Rule("health.required", domain="domain:health")
    )
    ...
    composition = DomainComposition(
        ...
        effective_profile=EffectiveReasoningProfile(
            base_profile="HealthProfile",
            required_rules=("health.required",),
    ...
    assert {conflict.code for conflict in plan.conflicts} == {
        "GLOBAL_MANDATORY_RULE_PROHIBITED",
...
def test_optional_disabled_permission_and_wrong_domain_are_partial_with_codes() -> None:
    ...
    plan = selector().select(
        registry=registry,
        profile=profile(
            optional=("health.disabled", "health.permission", "project.wrong")
        ),
        effective_permissions=(),
    )
    assert plan.status is DomainRuleSelectionStatus.PARTIAL
    assert set(plan.omitted_rule_ids) == {
        "health.disabled",
        "health.permission",
        "project.wrong",
    }
...
def test_composed_rules_are_selected_with_original_10_8_provenance() -> None:
    ...
    composition = DomainComposition(
        ...
        status="composed",
        primary_domain=DomainId("health"),
        supporting_domains=(DomainId("project"),),
        rules=(composed_item,),
        composed_at=NOW,
    )
    plan = selector().select(
