...
def _request(
    action: PermissionCapability, **changes: object
) -> DomainPermissionRequest:
    values: dict[str, object] = {
...
def test_remote_provider_respects_maximum_sensitivity() -> None:
    ...
    assert evaluate_domain_policy(policy, request).reasons == (
...
def test_local_provider_does_not_inherit_remote_policy() -> None:
    policy = _policy(
        PermissionCapability.MODEL_EXTERNAL, egress_policy=_egress_policy()
    )
    request = _request(
        PermissionCapability.MODEL_EXTERNAL,
    ...
    assert evaluate_domain_policy(policy, request).reasons == (
        "egress_provider_location_mismatch",
    )
...
def test_egress_purpose_and_provider_must_match() -> None:
    ...
    assert (
        evaluate_domain_policy(policy, wrong_provider).effect is PermissionOutcome.DENY
    )
    assert (
        evaluate_domain_policy(policy, wrong_purpose).effect is PermissionOutcome.DENY
    )
...
def test_export_does_not_include_unauthorized_identifiers() -> None:
    result = evaluate_domain_policy(
        ...
        _request(
            ...
            export_request=_export_request(identifiers=("personal_id",)),
        ),
    )
    assert result.effect is PermissionOutcome.DENY
    ...
    result = evaluate_domain_policy(
        ...
        _request(
            ...
            export_request=_export_request(
                content_kind=ExportContentKind.ORIGINAL_EVIDENCE
            ),
        ),
    )
    assert result.effect is PermissionOutcome.DENY
...
def test_read_permission_does_not_authorize_export() -> None:
    ...
    result = evaluate_domain_policy(
        _policy(PermissionCapability.RESOURCE_READ), request
    )
    assert result.effect is PermissionOutcome.DENY
...
def test_inference_permission_does_not_authorize_persistence_or_egress() -> None:
    ...
    persistence = evaluate_domain_policy(
        policy, _request(PermissionCapability.SENSITIVE_INFERENCE_PERSIST)
    )
    egress = evaluate_domain_policy(
        policy, _request(PermissionCapability.MODEL_EXTERNAL)
    )
...
def test_post_verification_requirement_round_trip() -> None:
    requirement = PostVerificationRequirement(
        kinds=(
            PostVerificationKind.DISPATCH_CONFIRMATION,
            PostVerificationKind.RESULT_EVIDENCE,
        ),
        resource_ids=("resource:brief",),
...
def test_export_expiration_fails_closed() -> None:
    ...
    result = evaluate_domain_policy(
        _policy(
            PermissionCapability.EXPORT,
            export_policy=_export_policy(expires_at=expired),
        ),
...
def test_approval_binds_exact_egress_purpose_provider_and_resources() -> None:
    ...
    request = _request(
        PermissionCapability.MODEL_EXTERNAL, egress_request=_egress_request()
    )
    result = evaluate_domain_policy(policy, request)
    assert result.effect is PermissionOutcome.APPROVAL_REQUIRED
    requirement = result.approval_requirements[0]
    assert (
        requirement.to_dict()["constraints"]["bound_egress"]
        == request.egress_request.to_dict()
    )
...
def test_consent_is_an_approval_obligation_not_a_caller_grant() -> None:
    ...
    assert (
        evaluate_domain_policy(policy, request).effect
...
def test_effect_capabilities_do_not_imply_distinct_effects() -> None:
    ...
    assert (
        evaluate_domain_policy(
            read_policy, _request(PermissionCapability.FILE_MODIFY)
        ).effect
        is PermissionOutcome.DENY
    )
    assert (
        evaluate_domain_policy(
            mutate_policy, _request(PermissionCapability.EXPORT)
        ).effect
        is PermissionOutcome.DENY
    )
    assert (
        evaluate_domain_policy(
            persist_policy, _request(PermissionCapability.MODEL_EXTERNAL)
        ).effect
        is PermissionOutcome.DENY
    )
    assert (
        evaluate_domain_policy(
            internal_policy, _request(PermissionCapability.COMMUNICATION_EXTERNAL)
        ).effect
        is PermissionOutcome.DENY
    )
