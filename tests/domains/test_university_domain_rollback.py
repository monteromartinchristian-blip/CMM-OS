"""Tests for Phase 10.22 University Domain atomic rollback after
...
def _assert_no_university_entries(registries):
    assert registries["domain_registry"].get(UNIVERSITY_DOMAIN_ID) is None
    assert registries["profile_registry"].get("university.profile") is None
...
def test_failure_after_definition_rolls_back():
    ...
    _assert_no_university_entries(registries)
