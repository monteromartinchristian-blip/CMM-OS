...
def _request() -> DomainTraceAssemblyRequest:
    ...
    supporting = DomainTraceContribution(
        ...
        (
            DomainTraceReference(
                "warning:1", DomainTraceReferenceKind.WARNING, "domain:health"
            ),
        ),
    )
...
def test_contract_round_trip_is_final_frozen_and_reference_only() -> None:
    ...
    assert not hasattr(
        __import__("cmm.domains.trace_contracts", fromlist=["*"]),
        "CrossDomain" + "TransferTrace",
    )
...
def test_contributions_require_one_exact_primary_and_every_supporting_domain() -> None:
    request = _request()
    duplicate_primary = DomainTraceContribution(
        "domain:health", DomainTraceRole.PRIMARY
    )
    ...
    with pytest.raises(DomainTraceContractError):
        DomainTraceAssemblyRequest(
            ...
            contributions=(request.contributions[0], duplicate_primary),
...
def test_request_rejects_a_primary_or_supporting_contribution_with_the_wrong_role() -> (
    None
):
    request = _request()
    inverted = (
...
def test_new_domain_owned_semantic_references_round_trip(kind_value: str) -> None:
    ...
    reference = DomainTraceReference(f"{kind_value}:1", kind, "domain:languages")
...
def test_inventory_rejects_duplicate_result_pairing_ids() -> None:
    pairing = DomainResultTraceReference(
        "domain-result:1", "domain:life-plan", "domain-trace:1"
    )

    with pytest.raises(DomainTraceContractError):
        DomainTraceReferenceInventory(
            references=(),
            domain_results=(pairing, pairing),
...
@pytest.mark.parametrize(
    ...
    (
        lambda data: data.update(
            supporting_domains=["domain:health", "domain:life-plan"]
        ),
        ...
        lambda data: data["contributions"].append(
            {"domain_id": "domain:foreign", "role": "supporting", "references": []}
        ),
