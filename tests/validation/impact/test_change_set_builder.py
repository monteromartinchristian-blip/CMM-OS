...
def test_change_set_builder_from_snapshots_detects_modified_and_renamed_files(tmp_path: Path) -> None:
    ...
    renamed = [item for item in change_set.file_changes if item.kind == FileChangeKind.RENAMED]
    modified = [item for item in change_set.file_changes if item.kind == FileChangeKind.MODIFIED]

    assert change_set.change_type in {ChangeType.RENAMED_FILE, ChangeType.STRUCTURAL_CHANGE}
...
def test_change_set_builder_from_explicit_changed_files(tmp_path: Path) -> None:
    ...
    assert tuple(str(path) for path in change_set.changed_files) == ("README.md", "pkg/changed.py")
