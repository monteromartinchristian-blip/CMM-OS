#!/usr/bin/env python3
"""Phase 10.49 — baseline-aware Ruff/format no-regression verifier.

Compares whole-repository Ruff lint debt and format debt between a pinned
pre-10.49 baseline and the current working tree, and additionally requires every
...
The comparison fails closed when the measured Ruff version differs from the
pinned ``EXPECTED_RUFF_VERSION``, because lint and format results are not
comparable across Ruff releases.

Reading this report truthfully:

* ``BASELINE_AWARE_GATE=PASS`` means the Phase 10.49 work added no new Ruff or
  format debt. It does not mean the whole repository is globally Ruff-clean.
...
# Phase 10.49 baseline: the committed implementation-plan HEAD from which Phase
# 10.49 work started cleanly.
BASELINE_HEAD = "6f9deeb37b6e3237bea4564c5e5607249eb12b70"

# Lint/format results are only comparable under an identical Ruff release.
...
def ruff_version_matches(measured: str, expected: str = EXPECTED_RUFF_VERSION) -> bool:
    """Return whether ``measured`` names the pinned Ruff release."""
    tokens = measured.strip().split()
    if not tokens:
        return False
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Verify that Phase 10.49 changes introduced no new repository Ruff or "
    ...
    try:
        ...
        if not ruff_version_matches(ruff_version):
            print(
                f"ERROR=ruff version mismatch: measured {ruff_version!r}, "
                f"pinned {EXPECTED_RUFF_VERSION!r}",
                file=sys.stderr,
            )
            return 2

        with tempfile.TemporaryDirectory(prefix="phase-10-49-baseline-") as tmp:
