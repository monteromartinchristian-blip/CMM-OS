#!/usr/bin/env python3
"""Generate deterministic Phase 10.34 V10 source, node, and gate evidence."""
...
SOURCE_MANIFEST = EVIDENCE_DIR / "phase-10.34-v10-source-hashes.json"
NODE_INVENTORY = EVIDENCE_DIR / "phase-10.34-v10-pytest-nodes.txt"
GATES = EVIDENCE_DIR / "phase-10.34-v10-gates.json"
...
def main() -> int:
    ...
    quality_scope = (
        "cmm/domains/session_*.py cmm/domains/resource_authority.py "
        "cmm/domains/knowledge_authority.py cmm/domains/composer.py "
        "cmm/domains/composition_contracts.py "
        "tests/domains/test_domain_session_*.py "
        "tests/domains/domain_session_audit_evidence.py "
        "tests/domains/domain_session_lifecycle_test_support.py scripts/audit/*.py"
    )
    gate_map = {
        "focused_tests": _gate(
            "focused_tests",
        ...
        "ruff_check": _gate(
            "ruff_check", f".venv/bin/ruff check {quality_scope}", binding
        ),
        "ruff_format": _gate(
            "ruff_format",
            f".venv/bin/ruff format --check {quality_scope}",
            binding,
        ),
        "compileall": _gate(
            "compileall",
            ".venv/bin/python -m compileall -q cmm tests scripts",
            binding,
        ),
    ...
    _write_json(
        GATES,
        {
            ...
            "pytest_evidence": {
                "inventory": "docs/audits/evidence/phase-10.34-v10-pytest-nodes.txt",
            ...
            "source_evidence": {
                "file_count": len(files),
                "manifest": "docs/audits/evidence/phase-10.34-v10-source-hashes.json",
