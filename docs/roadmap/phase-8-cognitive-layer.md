...
**Status: implemented** (`cmm/cognitive/privacy.py`, `tests/cognitive/test_privacy_metadata.py`).

8.25 keeps three concepts explicitly separate rather than merging them into one contract: **sensitivity** (content criticality — reused as-is from `SensitivityLevel`), **permissions** (who may do what — reused as-is from `ResourcePermission`/`ResourcePermissionOperation`), and **processing policy** (where content may be processed, whether it may leave the local runtime, be cached, be exported, or requires redaction/approval — the new `PrivacyMetadata` contract). No existing sensitivity or permission contract was duplicated.

`PrivacyMetadata` is immutable, versioned (`schema_version`), and JSON-serializable (`serialize`/`to_dict`/`from_mapping`/`from_dict`). `resolve_effective_privacy_metadata(*policies)` combines any number of sources and always keeps the strongest restriction: most-restrictive `policy`, highest `sensitivity`, intersected processing locations and provider allowlists, unioned prohibited providers, AND of every permissive boolean (`allow_remote`/`allow_premium`/`allow_cache`/`allow_export`), OR of every requirement flag (`requires_redaction`/`requires_approval`), and permissions that only ever narrow, never widen. A descendant can never relax what an ancestor restricted. `evaluate_privacy_operation(privacy, operation, context)` is a small, pure evaluator over `PROCESS_LOCAL`/`PROCESS_REMOTE`/`CACHE`/`EXPORT`/`USE_PREMIUM`/`TRANSMIT_TO_PROVIDER` that returns a structured `PrivacyDecision` (never a bare boolean) distinguishing allowed, denied-with-reason-code, redaction-required, and approval-required outcomes; `ResourcePermission` is still consulted for actor/domain/expiration on every operation.

`PrivacyMetadata` guarantees `LOCAL_ONLY` content structurally cannot declare `allow_remote=True` or include `REMOTE` in its own allowed processing locations, so it can never leave the local runtime through this contract.

Propagation in this phase is minimal and explicit, not a rewrite of existing contracts: `privacy_from_resource`/`privacy_from_knowledge_item`/`privacy_from_knowledge_package` (in `cmm/cognitive/privacy.py`) are pure helpers callers invoke explicitly — they do not walk the Knowledge Store or the extraction pipeline. `Resource` and `KnowledgeItem` were **not** modified (avoiding a `privacy.py` ↔ `resources.py`/`knowledge.py` import cycle and any risk to their existing test suites); their existing `sensitivity`/`permissions` are read directly by the helpers instead. `KnowledgePackage.privacy` (the generic mapping introduced in 8.23) is left untouched at the contract level for full backward compatibility with historical payloads, and is only opportunistically parsed as a serialized `PrivacyMetadata` by `privacy_from_knowledge_package` when it looks like one; unstructured mappings (e.g. `{"classification": "internal"}`) are safely ignored rather than raising. `CognitiveCacheEntry` gained an optional `privacy: PrivacyMetadata | None` field (alongside the pre-8.25 `sensitivity`/`permissions`, kept for compatibility); `CognitiveCache.put` now rejects storing an entry whose privacy forbids caching (`allow_cache=False`), and `CognitiveCache.get` will not return a `LOCAL_ONLY` entry to a context whose new `processing_location` is `REMOTE`. `cache_entry_from_knowledge_package` derives an entry's privacy from its package and resolves it against any explicit override with `resolve_effective_privacy_metadata`, so a cache entry can never be less restrictive than the package it was built from.

8.26 will integrate this metadata into the Phase 7 Validation Pipeline / Structural Cognitive Validation. Phase 10 will add domain-specific policies on top of this foundation. Phase 11 will apply this privacy metadata to provider routing, export, and global auditing of provider calls — none of which exist yet and are explicitly out of scope here (see "Out of scope" in `cmm/cognitive/privacy.py`'s module docstring).
