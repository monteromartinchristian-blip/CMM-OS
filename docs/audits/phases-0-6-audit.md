...
| Fase 6 - Transformaciones arquitectónicas | ⚠️ Parcial | 15/18 = 83% | Infraestructura 6.1, `MoveFunctionTransformation`, `MoveClassTransformation`, `ExtractMethodTransformation` y `ExtractModuleTransformation` E2E | Hitos 6.1-6.4 cerrables dentro de sus alcances; siguen pendientes análisis de impacto/referencias avanzadas y reorganización. El porcentaje se mantiene porque F6-10, F6-17 y F6-18 siguen incompletos. |
...
| `.venv/bin/python -m pytest -q` | `530 passed in 11.47s` tras la revisión de 6.4. |
...
| `.venv/bin/python -m pytest -q tests/transformations tests/execution` | `227 passed in 9.87s` tras la revisión de 6.4. |
...
| F6-06 | Ejecución | ✅ Implementado y funcional para infraestructura 6.1 y transformaciones 6.2-6.4 | `cmm/execution/execution_pipeline.py`; transformaciones y executors Python | Tests E2E de `move_function`, `move_class`, `extract_method` y `extract_module` | Ejecuta secuencialmente con registry, detiene en primer fallo y aplica extracción/movimiento real con rollback. |
...
| F6-09 | Reutilización de operaciones anteriores | ✅ Implementado y funcional para 6.1-6.4 | Operaciones tipadas, executors LibCST y operaciones específicas `extract_method`/`extract_module` | Tests unitarios y E2E de las cuatro transformaciones | Reutiliza pipeline, validación, snapshots y actualización de imports; usa operaciones específicas cuando la semántica no puede expresarse como copia/borrado simple. |
...
| F6-11 | Actualización de imports | ✅ Implementado y funcional en el alcance soportado de 6.2-6.4 | `update_imports_executor.py`; transformers de import simple y multi-símbolo | E2E de movimientos y extracción de módulo con consumidores/reexports | Actualiza imports simples y multi-símbolo seleccionado, conserva alias y reexports; imports directos, relativos, wildcard, dinámicos y ambiguos se rechazan antes de mutar. |
| F6-12 | Resolución de referencias | ⚠️ Parcial | `reference_index.py`; `import_resolver.py`; `execution_context.py`; precondiciones de extracción | E2E de movimientos y extracciones | Soporta referencias/imports estáticos declarados y rechaza formas ambiguas; sigue sin análisis global, imports cualificados o referencias dinámicas. |
...
| F6-16 | Tests unitarios e integración | ✅ Implementado y funcional para infraestructura 6.1 y transformaciones 6.2-6.4 | Tests de DAG, precondiciones, executors, pipeline y E2E reales de las cuatro transformaciones | `tests/transformations tests/execution` -> `227 passed`; suite global -> `530 passed` | Incluye extracción simple/async/con salida, `self`/`cls`, shadowing y flujo rechazado, módulos nuevos, dependencias seleccionadas, conflictos, imports rechazados, validación y rollback byte a byte. |
| F6-17 | Transformaciones enumeradas | ⚠️ Parcial | Existen `move_function`, `move_class`, `extract_method` y `extract_module` | Tests E2E de las cuatro; sin reorganización | Quedan reorganización, división, fusión y renombrado de módulos/paquetes. |
| F6-18 | Transformación arquitectónica completa validada | ⚠️ Parcial | Cuatro transformaciones E2E integradas en la infraestructura 6.1 | Tests E2E reales de 6.2-6.4 | `move_function`, `move_class`, `extract_method` y `extract_module` están validados en alcances conservadores; faltan impacto global y reorganización de módulos/paquetes. |
...
Veredicto de Fase 6 completa: no puede cerrarse oficialmente. Siguen ausentes o parciales el análisis de impacto, la resolución de referencias más allá del subconjunto seguro y la reorganización de módulos/paquetes.
...
| Fase 6.1, infraestructura E2E exitosa | `.venv/bin/python -m pytest -q tests/execution/test_phase_6_execution_pipeline.py::test_e2e_real_successful_dag_with_preconditions` | Proyecto Python temporal, DAG con dependencia, precondición válida, ejecución real `ExecutionPlanner -> ExecutionPipeline -> OperationExecutorRegistry`, validación final | Incluido en `227 passed`; crea `alpha.py` y `pkg/beta.py`, validación final exitosa | Archivos temporales de `tmp_path` | Demuestra flujo integrado sin mocks para el camino feliz de 6.1. |
| Fase 6.1, rollback E2E byte a byte | `.venv/bin/python -m pytest -q tests/execution/test_phase_6_execution_pipeline.py::test_e2e_real_failure_restores_bytes` | Crear archivo, modificar módulo real, fallar después y restaurar todo byte a byte | Incluido en `227 passed`; rollback completo | Archivos temporales de `tmp_path` | Demuestra rollback no-Git de 6.1. |
| Fase 6.1, validación final con rollback | `.venv/bin/python -m pytest -q tests/execution/test_phase_6_execution_pipeline.py::test_final_validation_failure_triggers_rollback` | Una operación escribe Python inválido, la validación falla y dispara rollback | Incluido en `227 passed`; rollback y validación restaurada correctos | `broken.py` temporal | Demuestra validación final transaccional. |
| Fase 6.2, move_function exitoso | `.venv/bin/python -m pytest -q tests/execution/test_move_function_e2e.py::test_move_function_simple_e2e_updates_consumer_and_deletes_source` | Mover `foo` entre módulos existentes y actualizar consumidor | Incluido en `227 passed`; fuente sin `foo`, destino con `foo`, consumidor y `__init__.py` actualizados, proyecto válido | Paquete Python temporal | E2E real sin mocks desde `MoveFunctionTransformation` hasta executors y validación. |
| Fase 6.2, semántica y alias | `.venv/bin/python -m pytest -q tests/execution/test_move_function_e2e.py::test_move_function_preserves_async_decorator_docstring_and_annotations tests/execution/test_move_function_e2e.py::test_move_function_supports_simple_from_import_alias` | Preservar async/decorador/docstring/anotaciones y alias simple | Incluido en `227 passed`; contenido preservado y alias mantenido | Paquete Python temporal | Alcance soportado verificado. |
| Fase 6.2, consumidores e imports | `.venv/bin/python -m pytest -q tests/execution/test_move_function_e2e.py::test_move_function_updates_multiple_consumers_and_multiline_import` | Actualizar múltiples consumidores e import multilinea | Incluido en `227 passed`; ambos consumidores actualizados | Paquete Python temporal | Reexport en `__init__.py` cubierto por el E2E simple. |
| Fase 6.2, precondiciones y referencias no soportadas | `.venv/bin/python -m pytest -q tests/execution/test_move_function_e2e.py::test_move_function_conflict_fails_before_mutation tests/execution/test_move_function_e2e.py::test_move_function_rejects_unsupported_direct_import_before_mutation tests/execution/test_move_function_e2e.py::test_move_function_rejects_unavailable_global_dependency` | Rechazar conflicto, `import module` ambiguo y global no disponible antes de mutar | Incluido en `227 passed`; errores estructurados y bytes intactos | Paquete Python temporal | Casos inseguros quedan explícitamente fuera del alcance. |
| Fase 6.2, rollback intermedio | `.venv/bin/python -m pytest -q tests/execution/test_move_function_e2e.py::test_move_function_rolls_back_after_real_import_update_failure tests/execution/test_move_function_e2e.py::test_move_function_rolls_back_after_real_delete_failure` | Fallar después de actualización/borrado reales | Incluido en `227 passed`; todos los archivos restaurados byte a byte | Paquete Python temporal | Rollback transaccional de `move_function` verificado. |
...
| Fase 6.4, extract_method E2E | `.venv/bin/python -m pytest -q tests/execution/test_extract_method_e2e.py` | Extraer bloques sin salida, con inputs, una salida, async, `self`/`cls`, decoradores, shadowing y validación fallida | `8 passed`; E2E real por planner/pipeline/executor, rollback byte a byte | Paquetes Python temporales | `extract_method` validado en alcance conservador. |
| Fase 6.4, extract_module E2E | `.venv/bin/python -m pytest -q tests/execution/test_extract_module_e2e.py` | Extraer función/clase, dependencias seleccionadas, destino nuevo, consumidores, reexports, conflictos, imports directos rechazados y rollback | `9 passed`; E2E real por planner/pipeline/executor, sin mocks | Paquetes Python temporales | `extract_module` validado en alcance conservador. |
...
| README dice que Phase 6 será el siguiente milestone en `README.md:L111-L117`. | Existen ya módulos `cmm/transformations`; `move_function` y `move_class` quedaron validados en sus alcances declarados, mientras la fase completa sigue parcial. | `tests/execution/test_move_function_e2e.py` y `tests/execution/test_move_class_e2e.py`; extract, impacto y reorganización siguen pendientes. |
...
Las cuatro transformaciones implementadas están validadas dentro de alcances conservadores, pero faltan análisis de impacto, referencias avanzadas y reorganización de módulos/paquetes.
...
1. Integrar análisis de impacto y ampliar la resolución de referencias.
2. Implementar reorganización, división, fusión y renombrado de módulos/paquetes.
3. Ejecutar la auditoría final de Fase 6.
