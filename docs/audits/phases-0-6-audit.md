...
| Fase 6 - Transformaciones arquitectónicas | ⚠️ Parcial | 14/18 = 78% | Infraestructura 6.1 y `MoveFunctionTransformation` E2E en `cmm/transformations/move_function.py`, executors Python y tests reales | Hitos 6.1 y 6.2 cerrables; siguen pendientes `move_class`, análisis de impacto/referencias avanzadas, extract y reorganización. |
...
| `.venv/bin/python -m pytest -q` | `488 passed in 3.30s` tras la revisión de 6.2. |
...
| `.venv/bin/python -m pytest -q tests/transformations tests/execution` | `185 passed in 2.36s` tras la revisión de 6.2. |
...
| F6-06 | Ejecución | ✅ Implementado y funcional para infraestructura 6.1 y `move_function` | `cmm/execution/execution_pipeline.py`; `cmm/transformations/move_function.py` | `tests/execution/test_phase_6_execution_pipeline.py`; `tests/execution/test_move_function_e2e.py` | Ejecuta secuencialmente con registry, detiene en primer fallo y mueve funciones reales con rollback. |
...
| F6-09 | Reutilización de operaciones anteriores | ✅ Implementado y funcional para 6.1/6.2 | `cmm/transformations/operations/copy_symbol.py`, `update_imports.py`, `delete_symbol.py`; executors Python LibCST | `tests/execution/test_move_function_e2e.py`; tests unitarios de executors | Reutiliza copia, actualización de imports, renombrado, borrado y validación sin duplicar runtimes semánticos. |
...
| F6-11 | Actualización de imports | ✅ Implementado y funcional en el alcance soportado de `move_function` | `cmm/transformations/operations/update_imports.py`; `cmm/execution/python/update_imports_executor.py`; `cmm/execution/python/visitors/update_import_transformer.py` | `tests/execution/python/test_update_imports_executor.py`; `tests/execution/test_move_function_e2e.py` | Actualiza `from module import symbol`, conserva alias y admite renombrado importado; imports directos, wildcard y referencias ambiguas se rechazan antes de mutar. |
...
| F6-16 | Tests unitarios e integración | ✅ Implementado y funcional para infraestructura 6.1 y `move_function` 6.2 | Tests de DAG, precondiciones, executors, pipeline y `tests/execution/test_move_function_e2e.py` | `tests/transformations tests/execution` -> `185 passed`; suite global -> `488 passed` | Incluye E2E sin mocks para éxito, async/decoradores, alias, múltiples consumidores, multilinea, conflictos, dependencias no soportadas y rollback intermedio. |
...
| F6-18 | Transformación arquitectónica completa validada | ⚠️ Parcial | `cmm/transformations/move_function.py`; `cmm/transformations/move_class.py` | `tests/execution/test_move_function_e2e.py`; `move_class` E2E sigue pendiente | `move_function` está validado en el alcance soportado; la Fase 6 aún no tiene `move_class` funcional ni las transformaciones restantes. |
...
Veredicto de Fase 6 completa: no puede cerrarse oficialmente. Siguen ausentes o parciales `move_class`, el análisis de impacto, la resolución de referencias más allá del subconjunto seguro de `move_function`, `extract_method`, `extract_module` y la reorganización de módulos/paquetes.
...
| Fase 6.1, infraestructura E2E exitosa | `.venv/bin/python -m pytest -q tests/execution/test_phase_6_execution_pipeline.py::test_e2e_real_successful_dag_with_preconditions` | Proyecto Python temporal, DAG con dependencia, precondición válida, ejecución real `ExecutionPlanner -> ExecutionPipeline -> OperationExecutorRegistry`, validación final | Incluido en `185 passed`; crea `alpha.py` y `pkg/beta.py`, validación final exitosa | Archivos temporales de `tmp_path` | Demuestra flujo integrado sin mocks para el camino feliz de 6.1. |
| Fase 6.1, rollback E2E byte a byte | `.venv/bin/python -m pytest -q tests/execution/test_phase_6_execution_pipeline.py::test_e2e_real_failure_restores_bytes` | Crear archivo, modificar módulo real, fallar después y restaurar todo byte a byte | Incluido en `185 passed`; rollback completo | Archivos temporales de `tmp_path` | Demuestra rollback no-Git de 6.1. |
| Fase 6.1, validación final con rollback | `.venv/bin/python -m pytest -q tests/execution/test_phase_6_execution_pipeline.py::test_final_validation_failure_triggers_rollback` | Una operación escribe Python inválido, la validación falla y dispara rollback | Incluido en `185 passed`; rollback y validación restaurada correctos | `broken.py` temporal | Demuestra validación final transaccional. |
| Fase 6.2, move_function exitoso | `.venv/bin/python -m pytest -q tests/execution/test_move_function_e2e.py::test_move_function_simple_e2e_updates_consumer_and_deletes_source` | Mover `foo` entre módulos existentes y actualizar consumidor | Incluido en `185 passed`; fuente sin `foo`, destino con `foo`, consumidor y `__init__.py` actualizados, proyecto válido | Paquete Python temporal | E2E real sin mocks desde `MoveFunctionTransformation` hasta executors y validación. |
| Fase 6.2, semántica y alias | `.venv/bin/python -m pytest -q tests/execution/test_move_function_e2e.py::test_move_function_preserves_async_decorator_docstring_and_annotations tests/execution/test_move_function_e2e.py::test_move_function_supports_simple_from_import_alias` | Preservar async/decorador/docstring/anotaciones y alias simple | Incluido en `185 passed`; contenido preservado y alias mantenido | Paquete Python temporal | Alcance soportado verificado. |
| Fase 6.2, consumidores e imports | `.venv/bin/python -m pytest -q tests/execution/test_move_function_e2e.py::test_move_function_updates_multiple_consumers_and_multiline_import` | Actualizar múltiples consumidores e import multilinea | Incluido en `185 passed`; ambos consumidores actualizados | Paquete Python temporal | Reexport en `__init__.py` cubierto por el E2E simple. |
| Fase 6.2, precondiciones y referencias no soportadas | `.venv/bin/python -m pytest -q tests/execution/test_move_function_e2e.py::test_move_function_conflict_fails_before_mutation tests/execution/test_move_function_e2e.py::test_move_function_rejects_unsupported_direct_import_before_mutation tests/execution/test_move_function_e2e.py::test_move_function_rejects_unavailable_global_dependency` | Rechazar conflicto, `import module` ambiguo y global no disponible antes de mutar | Incluido en `185 passed`; errores estructurados y bytes intactos | Paquete Python temporal | Casos inseguros quedan explícitamente fuera del alcance. |
| Fase 6.2, rollback intermedio | `.venv/bin/python -m pytest -q tests/execution/test_move_function_e2e.py::test_move_function_rolls_back_after_real_import_update_failure tests/execution/test_move_function_e2e.py::test_move_function_rolls_back_after_real_delete_failure` | Fallar después de actualización/borrado reales | Incluido en `185 passed`; todos los archivos restaurados byte a byte | Paquete Python temporal | Rollback transaccional de `move_function` verificado. |
...
| README dice que Phase 6 será el siguiente milestone en `README.md:L111-L117`. | Existen ya módulos `cmm/transformations`; `move_function` quedó validado en 6.2, mientras la fase completa sigue parcial. | La evidencia actual es `tests/execution/test_move_function_e2e.py`; `move_class` y las transformaciones restantes siguen pendientes. |
...
1. Implementar y validar `move_class` E2E.
2. Integrar análisis de impacto y ampliar la resolución de referencias.
3. Implementar `extract_method` y `extract_module`.
4. Implementar reorganización, división, fusión y renombrado de módulos/paquetes.
5. Ejecutar la auditoría final de Fase 6.
