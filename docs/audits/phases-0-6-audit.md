...
| Fase 6 - Transformaciones arquitectónicas | ⚠️ Parcial | 15/18 = 83% | Infraestructura 6.1, `MoveFunctionTransformation` y `MoveClassTransformation` E2E, executors tipados y tests reales | Hitos 6.1, 6.2 y 6.3 cerrables; siguen pendientes análisis de impacto/referencias avanzadas, extract y reorganización. |
...
| `.venv/bin/python -m pytest -q` | `507 passed in 7.96s` tras la revisión técnica de 6.3. |
...
| `.venv/bin/python -m pytest -q tests/transformations tests/execution` | `204 passed in 8.23s` tras la revisión técnica de 6.3. |
...
| F6-06 | Ejecución | ✅ Implementado y funcional para infraestructura 6.1, `move_function` y `move_class` | `cmm/execution/execution_pipeline.py`; `cmm/transformations/move_function.py`; `cmm/transformations/move_class.py` | `tests/execution/test_phase_6_execution_pipeline.py`; `tests/execution/test_move_function_e2e.py`; `tests/execution/test_move_class_e2e.py` | Ejecuta secuencialmente con registry, detiene en primer fallo y mueve funciones/clases reales con rollback. |
...
| F6-09 | Reutilización de operaciones anteriores | ✅ Implementado y funcional para 6.1/6.2/6.3 | `cmm/transformations/operations/copy_symbol.py`, `update_imports.py`, `delete_symbol.py`; locators y executors Python LibCST tipados | `tests/execution/test_move_function_e2e.py`; `tests/execution/test_move_class_e2e.py`; tests unitarios de executors | Reutiliza copia, actualización de imports, renombrado, borrado y validación para funciones y clases sin duplicar runtimes semánticos. |
...
| F6-11 | Actualización de imports | ✅ Implementado y funcional en el alcance soportado de `move_function` y `move_class` | `cmm/transformations/operations/update_imports.py`; `cmm/execution/python/update_imports_executor.py`; `cmm/execution/python/visitors/update_import_transformer.py` | `tests/execution/python/test_update_imports_executor.py`; `tests/execution/test_move_function_e2e.py`; `tests/execution/test_move_class_e2e.py` | Actualiza `from module import symbol`, conserva alias, herencia y reexports, y admite renombrado importado; imports directos, relativos, wildcard y referencias ambiguas se rechazan antes de mutar. |
| F6-12 | Resolución de referencias | ⚠️ Parcial | `cmm/execution/python/reference_index.py`; `cmm/execution/python/import_resolver.py`; `cmm/execution/execution_context.py` | `tests/execution/test_move_function_e2e.py`; `tests/execution/test_move_class_e2e.py` | Soporta referencias simples importadas de funciones/clases, alias, herencia y reexports; sigue sin análisis global, imports cualificados, dinámicos o ambiguos. |
...
| F6-16 | Tests unitarios e integración | ✅ Implementado y funcional para infraestructura 6.1, `move_function` 6.2 y `move_class` 6.3 | Tests de DAG, precondiciones, executors, pipeline, `tests/execution/test_move_function_e2e.py` y `tests/execution/test_move_class_e2e.py` | `tests/transformations tests/execution` -> `204 passed`; suite global -> `507 passed` | Incluye E2E sin mocks para éxito, estructuras complejas, alias, herencia, múltiples consumidores, conflictos, dependencias no soportadas, homónimos ambiguos, símbolos duplicados, validación y rollback. |
...
| F6-18 | Transformación arquitectónica completa validada | ⚠️ Parcial | `cmm/transformations/move_function.py`; `cmm/transformations/move_class.py` | `tests/execution/test_move_function_e2e.py`; `tests/execution/test_move_class_e2e.py` | `move_function` y `move_class` están validados en el alcance soportado; la Fase 6 aún carece de extract, análisis de impacto y reorganización de módulos/paquetes. |
...
Veredicto de Fase 6 completa: no puede cerrarse oficialmente. Siguen ausentes o parciales el análisis de impacto, la resolución de referencias más allá del subconjunto seguro, `extract_method`, `extract_module` y la reorganización de módulos/paquetes.
...
| Fase 6.1, infraestructura E2E exitosa | `.venv/bin/python -m pytest -q tests/execution/test_phase_6_execution_pipeline.py::test_e2e_real_successful_dag_with_preconditions` | Proyecto Python temporal, DAG con dependencia, precondición válida, ejecución real `ExecutionPlanner -> ExecutionPipeline -> OperationExecutorRegistry`, validación final | Incluido en `204 passed`; crea `alpha.py` y `pkg/beta.py`, validación final exitosa | Archivos temporales de `tmp_path` | Demuestra flujo integrado sin mocks para el camino feliz de 6.1. |
| Fase 6.1, rollback E2E byte a byte | `.venv/bin/python -m pytest -q tests/execution/test_phase_6_execution_pipeline.py::test_e2e_real_failure_restores_bytes` | Crear archivo, modificar módulo real, fallar después y restaurar todo byte a byte | Incluido en `204 passed`; rollback completo | Archivos temporales de `tmp_path` | Demuestra rollback no-Git de 6.1. |
| Fase 6.1, validación final con rollback | `.venv/bin/python -m pytest -q tests/execution/test_phase_6_execution_pipeline.py::test_final_validation_failure_triggers_rollback` | Una operación escribe Python inválido, la validación falla y dispara rollback | Incluido en `204 passed`; rollback y validación restaurada correctos | `broken.py` temporal | Demuestra validación final transaccional. |
| Fase 6.2, move_function exitoso | `.venv/bin/python -m pytest -q tests/execution/test_move_function_e2e.py::test_move_function_simple_e2e_updates_consumer_and_deletes_source` | Mover `foo` entre módulos existentes y actualizar consumidor | Incluido en `204 passed`; fuente sin `foo`, destino con `foo`, consumidor y `__init__.py` actualizados, proyecto válido | Paquete Python temporal | E2E real sin mocks desde `MoveFunctionTransformation` hasta executors y validación. |
| Fase 6.2, semántica y alias | `.venv/bin/python -m pytest -q tests/execution/test_move_function_e2e.py::test_move_function_preserves_async_decorator_docstring_and_annotations tests/execution/test_move_function_e2e.py::test_move_function_supports_simple_from_import_alias` | Preservar async/decorador/docstring/anotaciones y alias simple | Incluido en `204 passed`; contenido preservado y alias mantenido | Paquete Python temporal | Alcance soportado verificado. |
| Fase 6.2, consumidores e imports | `.venv/bin/python -m pytest -q tests/execution/test_move_function_e2e.py::test_move_function_updates_multiple_consumers_and_multiline_import` | Actualizar múltiples consumidores e import multilinea | Incluido en `204 passed`; ambos consumidores actualizados | Paquete Python temporal | Reexport en `__init__.py` cubierto por el E2E simple. |
| Fase 6.2, precondiciones y referencias no soportadas | `.venv/bin/python -m pytest -q tests/execution/test_move_function_e2e.py::test_move_function_conflict_fails_before_mutation tests/execution/test_move_function_e2e.py::test_move_function_rejects_unsupported_direct_import_before_mutation tests/execution/test_move_function_e2e.py::test_move_function_rejects_unavailable_global_dependency` | Rechazar conflicto, `import module` ambiguo y global no disponible antes de mutar | Incluido en `204 passed`; errores estructurados y bytes intactos | Paquete Python temporal | Casos inseguros quedan explícitamente fuera del alcance. |
| Fase 6.2, rollback intermedio | `.venv/bin/python -m pytest -q tests/execution/test_move_function_e2e.py::test_move_function_rolls_back_after_real_import_update_failure tests/execution/test_move_function_e2e.py::test_move_function_rolls_back_after_real_delete_failure` | Fallar después de actualización/borrado reales | Incluido en `204 passed`; todos los archivos restaurados byte a byte | Paquete Python temporal | Rollback transaccional de `move_function` verificado. |
...
`move_function` y `move_class` están validados dentro de sus alcances conservadores, pero faltan análisis de impacto, referencias avanzadas, extract y reorganización de módulos/paquetes.
...
2. Implementar `extract_method` y `extract_module`.
3. Implementar reorganización, división, fusión y renombrado de módulos/paquetes.
4. Ejecutar la auditoría final de Fase 6.
