...
| `DP-043` | Integration with Validation System: Domain Intelligence expresses specialized validation policies and validation obligations while all authoritative validation execution remains owned by the canonical Phase 7 Validation System and, for agentic runtime execution, the existing Phase 9 validation bridge; pack install/update, operations, workflows, cross-domain execution, specialized results, and Project Domain code changes fail closed when mandatory validation is missing, unknown, malformed, unavailable, or unsuccessful; no parallel validation infrastructure introduced. | `SRC-R10:R10-C43` | 10.43 | Policy bindings `cmm/domains/validation_policy_bindings.py` (six families); thin integration `cmm/domains/validation_integration.py`; pack bridge `cmm/domains/validation.py` (`PipelineDomainValidator` + `ensure_domain_validation_allows_install` / `ensure_domain_validation_allows_update`); lifecycle gate in `cmm/domains/loader.py` (`DeclarativeDomainLoader` enforces install/update policies before registration); runtime bridge `cmm/domains/operation_execution.py` (`DefaultDomainOperationOrchestrator` provider seam, `build_domain_workflow_operation_adapter`, `OrchestratedCrossDomainOperationPort`, specialized-result acceptance gate); generic Phase 9 seam `cmm/agent_runtime/operation_execution_contracts.py` + `cmm/agent_runtime/operation_execution_adapter.py` (`validation_requirements`, `validation_project_root`); reference `docs/reference/domain-validation-integration.md`; design `docs/superpowers/specs/2026-09-06-phase-10.43-integration-with-validation-system-design.md`; plan `docs/superpowers/plans/2026-09-06-phase-10.43-validation-system-integration-implementation-plan.md`; `DP-043=IMPLEMENTED_PENDING_INDEPENDENT_AUDIT`; `AT-DP-043=PASS_CONNECTED` (implementation evidence, independent audit pending) | `IMPLEMENTED_PENDING_INDEPENDENT_AUDIT` | `AT-DP-043` — `PASS_CONNECTED` (`tests/domains/test_domain_validation_integration_dp043_acceptance.py`; connected acceptance through real canonical paths including provider-omission fail-closed, empty-requirements fail-closed, provider-independent specialized gate, Project affected-test rejection with repair, and impact escalation via the canonical Project change policy; independent audit pending) |
