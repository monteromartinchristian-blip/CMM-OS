...
| `DP-022` | University: cross-workstream prioritization, workload, deadlines, academic risk, source separation, and institutional documents. Covers `REQ-DP22-001` (prioritization/workload/dependencies), `REQ-DP22-002` (deadlines, source authority, integrity), and `REQ-DP22-003` (concrete workflows) as sub-requirements of one canonical unit. | `SRC-P14:P14-C02` | 10.22 | shared domain infrastructure | `REQUIRES_PHASE_INSPECTION` | `AT-DP-022` |
