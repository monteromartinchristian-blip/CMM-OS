...
class ModelCostCalculator:
    ...
    def estimate(
        ...
        *,
        ...
        missing: list[str] = []
        input_cost = self._cost(
            input_tokens, model.input_cost_per_million, "input", missing
        )
        cached_cost = self._cost(
            cached_input_tokens,
            model.cached_input_cost_per_million,
            "cached_input",
            missing,
        )
        output_cost = self._cost(
            output_tokens, model.output_cost_per_million, "output", missing
        )
    ...
    @staticmethod
    def _cost(
        tokens: int, price: Decimal | None, label: str, missing: list[str]
    ) -> Decimal:
        if tokens == 0:
            return Decimal(0)
        if price is None:
