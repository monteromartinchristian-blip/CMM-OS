    ...
    return {
        ...
        "candidates": [
            {
                ...
                "input_cost_per_million": (
                    ...
                    if candidate.input_cost_per_million is not None else None
                ),
                "output_cost_per_million": (
                    str(candidate.output_cost_per_million)
                    if candidate.output_cost_per_million is not None else None
                ),
        ...
        "ranking_policy": {
            ...
            "preferred_providers": list(
                decision.ranking_policy.preferred_providers
            ),
        },
        ...
        candidates = tuple(
            RoutingCandidate(
                ...
                input_cost_per_million=(
                    ...
                    if item.get("input_cost_per_million") is not None else None
                ),
                output_cost_per_million=(
                    Decimal(str(item["output_cost_per_million"]))
                    if item.get("output_cost_per_million") is not None else None
                ),
...
class ModelAttemptResult:
    ...
    def __post_init__(self) -> None:
        if not all(isinstance(value, str) for value in (
            self.operation_id, self.model_id, self.provider_id,
        )):
            raise InvalidModelFallbackContractError("attempt identifiers must be strings")
        if not self.operation_id.strip() or not self.model_id.strip() or not self.provider_id.strip():
            raise InvalidModelFallbackContractError("attempt identifiers must be non-empty")
        ...
        for name in ("estimated_cost", "actual_cost"):
            ...
            if value is not None and (not isinstance(value, Decimal) or value < 0):
                raise InvalidModelFallbackContractError(f"{name} must be a non-negative Decimal")
        if self.latency_ms is not None and (
            not isinstance(self.latency_ms, int)
            or isinstance(self.latency_ms, bool)
            or self.latency_ms < 0
        ):
            raise InvalidModelFallbackContractError("latency_ms must be a non-negative integer")
        ...
        object.__setattr__(self, "trigger", _enum(self.trigger, ModelFallbackTrigger, "trigger"))
        object.__setattr__(self, "validation", _freeze(self.validation))
        object.__setattr__(self, "metadata", _freeze(self.metadata))

    def to_dict(self) -> dict[str, Any]:
        return _json({"operation_id": self.operation_id, "attempt_index": self.attempt_index,
                      ...
                      "latency_ms": self.latency_ms, "validation": self.validation,
                      "metadata": self.metadata})

    @classmethod
    def from_dict(cls, data: Mapping[str, Any]) -> ModelAttemptResult:
        values = dict(data)
        values["trigger"] = _enum(values.get("trigger"), ModelFallbackTrigger, "trigger")
...
class ModelAttemptHistory:
    ...
    def __post_init__(self) -> None:
        ...
        if any(not isinstance(a, ModelAttemptResult) for a in attempts):
            raise InvalidModelFallbackContractError("history must contain attempt results")
        seen: set[tuple[str, int, str, str]] = set()
        expected_index = 1
        operation_id: str | None = None
        for attempt in attempts:
            key = (attempt.operation_id, attempt.attempt_index,
    ...
    def from_dict(cls, data: Mapping[str, Any]) -> ModelAttemptHistory:
        return cls(tuple(ModelAttemptResult.from_dict(item) for item in data.get("attempts", ())))
...
class ModelFallbackPolicy:
    ...
    retryable_triggers: tuple[ModelFallbackTrigger, ...] = (
        ModelFallbackTrigger.TIMEOUT, ModelFallbackTrigger.RATE_LIMIT,
        ModelFallbackTrigger.TRANSIENT_ERROR,
    )
    escalation_triggers: tuple[ModelFallbackTrigger, ...] = (
        ModelFallbackTrigger.PRIVACY_INCOMPATIBLE, ModelFallbackTrigger.BUDGET_EXHAUSTED,
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.id, str) or not self.id.strip() or not isinstance(self.version, str) or not self.version.strip():
            raise InvalidModelFallbackContractError("policy id and version must be non-empty strings")
        limits = (self.maximum_attempts, self.maximum_attempts_per_model, self.maximum_attempts_per_provider)
        if any(not isinstance(value, int) or isinstance(value, bool) or value < 1 for value in limits):
            raise InvalidModelFallbackContractError("fallback policy identifiers and limits are invalid")
        object.__setattr__(self, "id", self.id.strip())
        object.__setattr__(self, "retryable_triggers", tuple(_enum(v, ModelFallbackTrigger, "retryable_triggers") for v in self.retryable_triggers))
        object.__setattr__(self, "escalation_triggers", tuple(_enum(v, ModelFallbackTrigger, "escalation_triggers") for v in self.escalation_triggers))
        object.__setattr__(self, "actions", tuple(_enum(v, ModelFallbackAction, "actions") for v in self.actions))
        if len(self.retryable_triggers) != len(set(self.retryable_triggers)) or len(self.escalation_triggers) != len(set(self.escalation_triggers)):
        ...
        for name in ("exclude_failed_model", "exclude_failed_provider", "allow_rerouting", "allow_requirement_modification", "allow_premium_with_approval", "pause_on_escalation"):
            if not isinstance(getattr(self, name), bool):
                raise InvalidModelFallbackContractError(f"{name} must be a bool")
        object.__setattr__(self, "metadata", _freeze(self.metadata))

    def to_dict(self) -> dict[str, Any]:
        return _json({"id": self.id, "version": self.version, "maximum_attempts": self.maximum_attempts,
                      ...
                      "retryable_triggers": self.retryable_triggers, "escalation_triggers": self.escalation_triggers,
                      "actions": self.actions, "exclude_failed_model": self.exclude_failed_model,
                      "exclude_failed_provider": self.exclude_failed_provider, "allow_rerouting": self.allow_rerouting,
                      ...
                      "pause_on_escalation": self.pause_on_escalation, "metadata": self.metadata})
...
class ModelFallbackContext:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.operation_id, str) or not self.operation_id.strip() or not isinstance(self.workflow_id, str) or not self.workflow_id.strip():
            raise InvalidModelFallbackContractError("context operation_id and workflow_id must be non-empty")
        if not isinstance(self.history, ModelAttemptHistory):
            raise InvalidModelFallbackContractError("history must be a ModelAttemptHistory")
        if not isinstance(self.policy, ModelFallbackPolicy):
            raise InvalidModelFallbackContractError("policy must be a ModelFallbackPolicy")
        if self.routing_decision is not None and not isinstance(self.routing_decision, RoutingDecision):
            raise InvalidModelFallbackContractError("routing_decision must be a RoutingDecision or None")
        ...
        if not isinstance(self.effective_requirements, ModelRequirements):
            raise InvalidModelFallbackContractError("effective_requirements must be ModelRequirements")
        if not isinstance(self.latest_result, ModelAttemptResult):
            raise InvalidModelFallbackContractError("latest_result must be ModelAttemptResult")
        if self.latest_result.operation_id != self.operation_id:
            raise InvalidModelFallbackContractError("latest_result operation_id conflicts with context")
        if self.history.attempts and self.history.attempts[-1] != self.latest_result:
            raise InvalidModelFallbackContractError("latest_result must be the final history attempt")
        for name in ("approval", "budget", "privacy", "policy_context", "metadata"):
            object.__setattr__(self, name, _freeze(getattr(self, name)))

    def to_dict(self) -> dict[str, Any]:
        return _json({"operation_id": self.operation_id, "workflow_id": self.workflow_id,
                      "routing_decision": _routing_to_dict(self.routing_decision) if self.routing_decision else None, "effective_requirements": model_requirements_to_dict(self.effective_requirements),
                      "latest_result": self.latest_result, "history": self.history,
                      "policy": self.policy, "approval": self.approval, "budget": self.budget,
                      "privacy": self.privacy, "policy_context": self.policy_context, "metadata": self.metadata})

    @classmethod
    def from_dict(cls, data: Mapping[str, Any]) -> ModelFallbackContext:
        return cls(
            operation_id=str(data["operation_id"]), workflow_id=str(data["workflow_id"]),
            routing_decision=_routing_from_dict(data["routing_decision"]) if data.get("routing_decision") else None,
            effective_requirements=model_requirements_from_dict(data["effective_requirements"]),
            ...
            approval=data.get("approval", {}), budget=data.get("budget", {}),
            privacy=data.get("privacy", {}), policy_context=data.get("policy_context", {}),
...
class ModelFallbackDecision:
    ...
    def __post_init__(self) -> None:
        object.__setattr__(self, "action", _enum(self.action, ModelFallbackAction, "action"))
        object.__setattr__(self, "trigger", _enum(self.trigger, ModelFallbackTrigger, "trigger"))
        object.__setattr__(self, "skipped_candidates", tuple(self.skipped_candidates))
        object.__setattr__(self, "reason_codes", tuple(self.reason_codes))
        object.__setattr__(self, "metadata", _freeze(self.metadata))
        if not isinstance(self.operation_id, str) or not self.operation_id.strip() or not isinstance(self.attempt_index, int) or isinstance(self.attempt_index, bool) or self.attempt_index < 1:
            raise InvalidModelFallbackContractError("decision operation_id and attempt_index are invalid")
        valid_strategies = {"retry", "retry_with_modified_parameters", "reobserve", "rerun_validation", "replan", "request_approval", "escalate", "pause", "fail"}
        if self.recovery_strategy is not None and self.recovery_strategy not in valid_strategies:
            raise InvalidModelFallbackContractError("invalid recovery_strategy")
        selections = {ModelFallbackAction.NEXT_ROUTING_CANDIDATE, ModelFallbackAction.SELECT_EQUIVALENT_MODEL, ModelFallbackAction.SELECT_LOWER_COST_MODEL, ModelFallbackAction.SELECT_HIGHER_QUALITY_MODEL}
        if self.action in selections and (not self.selected_model_id or not self.selected_provider_id):
            raise InvalidModelFallbackContractError("selection actions require a selected model and provider")
        if self.action is ModelFallbackAction.REQUEST_APPROVAL and not self.requires_approval:
            raise InvalidModelFallbackContractError("REQUEST_APPROVAL requires requires_approval")
        ...
        if not self.idempotency_key:
            payload = {
                "operation_id": self.operation_id, "attempt_index": self.attempt_index,
    ...
    def to_dict(self) -> dict[str, Any]:
        return _json({"operation_id": self.operation_id, "attempt_index": self.attempt_index,
                      "action": self.action, "trigger": self.trigger, "selected_model_id": self.selected_model_id,
                      "selected_provider_id": self.selected_provider_id, "skipped_candidates": self.skipped_candidates,
                      "reason_codes": self.reason_codes, "effective_requirements": model_requirements_to_dict(self.effective_requirements) if self.effective_requirements else None,
                      "requires_approval": self.requires_approval, "pause": self.pause,
                      "recovery_strategy": self.recovery_strategy, "idempotency_key": self.idempotency_key,
                      "metadata": self.metadata})
