...
class EconomicBudgetResolver:
    def resolve(
        ...
        *,
        ...
        sources = tuple(x for x in (goal, workflow, operation, policy, approval) if x is not None)
        if not sources:
            raise EconomicBudgetResolutionError("at least one economic budget source is required")
        currencies = {source.currency for source in sources}
        if len(currencies) != 1:
            raise EconomicBudgetResolutionError("economic budget sources must use the same currency")

        def minimum(name: str):
            values = [getattr(source, name) for source in sources if getattr(source, name) is not None]
        ...
        for source in sources:
            for key, value in source.metadata.items():
                ...
                metadata[key] = value
        if any(source.source.value == "policy" and source.status is EconomicBudgetStatus.DENIED for source in sources):
            reasons.append("budget.policy_denied")
        return ResolvedEconomicBudget(
            maximum_cost=minimum("maximum_cost"),
            maximum_estimated_cost_per_operation=minimum("maximum_estimated_cost_per_operation"),
            maximum_actual_cost_per_operation=minimum("maximum_actual_cost_per_operation"),
            ...
            warning_threshold_percent=min(source.warning_threshold_percent for source in sources),
            critical_threshold_percent=min(source.critical_threshold_percent for source in sources),
            on_warning=self._most_restrictive_action(source.on_warning for source in sources),
            on_exhaustion=self._most_restrictive_action(source.on_exhaustion for source in sources),
            allow_overrun_with_approval=all(source.allow_overrun_with_approval for source in sources),
            savings_mode=any(source.savings_mode for source in sources),
            provenance=tuple(f"{source.source.value}:{source.id}" for source in sources),
            reason_codes=tuple(reasons),
            metadata=metadata,
            policy_denied=any(
                source.source.value == "policy" and source.status is EconomicBudgetStatus.DENIED
                for source in sources
            ),
        )
    ...
    def decide(
        ...
        *,
        ...
        if action_budget is not None:
            ...
            if action_budget.status in (ActionBudgetStatus.PAUSED, ActionBudgetStatus.CANCELLED):
                return EconomicBudgetDecision(
            ...
            if action_budget.status in (ActionBudgetStatus.EXHAUSTED, ActionBudgetStatus.COMPLETED):
                return EconomicBudgetDecision(
                    EconomicBudgetAction.DENY,
                    (f"budget.action_budget_{action_budget.status.value}", "budget.exhausted"),
                    replace(resolved, status=EconomicBudgetStatus.EXHAUSTED, exhausted=True),
                )
        ...
        if policy_denied or resolved.policy_denied:
            return EconomicBudgetDecision(
                ...
                replace(resolved, status=EconomicBudgetStatus.DENIED, policy_denied=True),
            )
        ...
        violations: list[str] = []
        if resolved.maximum_input_tokens is not None and estimate.input_tokens > resolved.maximum_input_tokens:
            violations.append("budget.input_tokens_exceeded")
        if resolved.maximum_output_tokens is not None and estimate.output_tokens > resolved.maximum_output_tokens:
            violations.append("budget.output_tokens_exceeded")
        if resolved.maximum_total_tokens is not None and estimate.total_tokens > resolved.maximum_total_tokens:
            violations.append("budget.total_tokens_exceeded")
        if resolved.maximum_estimated_cost_per_operation is not None and estimate.total_cost > resolved.maximum_estimated_cost_per_operation:
            violations.append("budget.estimated_cost_per_operation_exceeded")
        if violations:
            return EconomicBudgetDecision(
                EconomicBudgetAction.DENY,
                tuple(violations),
                replace(resolved, status=EconomicBudgetStatus.DENIED, estimated_cost_excessive=True, estimated_cost=estimate.total_cost),
            )
        ...
        if resolved.maximum_cost is not None:
            ...
            if estimate.total_cost > available_cost:
                return EconomicBudgetResolver._blocked(
                    resolved, "budget.total_cost_unavailable", "budget.exhausted",
                    estimated_cost_excessive=True, exhausted=True,
                )
        ...
        return EconomicBudgetDecision(
            action,
            tuple(reasons),
            replace(resolved, status=status, warning=warning, near_exhaustion=near, estimated_cost=estimate.total_cost),
        )
        ...
        *,
        ...
        if currency is not None and currency.upper() != resolved.currency.upper():
            return EconomicBudgetDecision(
                EconomicBudgetAction.DENY,
                ("budget.currency_conflict",),
                replace(resolved, status=EconomicBudgetStatus.DENIED, actual_cost=actual_cost, estimated_cost=estimated_cost),
            )
        if action_budget is not None:
            used_cost = Decimal(action_budget.used_for(BudgetResourceType.COST))
            reserved_cost = Decimal(action_budget.reserved_for(BudgetResourceType.COST))
            available_cost = action_budget.available_for(BudgetResourceType.COST)
            if currency is not None and action_budget.currency.upper() != currency.upper():
                return EconomicBudgetDecision(
                    EconomicBudgetAction.DENY,
                    ("budget.currency_conflict",),
                    replace(resolved, status=EconomicBudgetStatus.DENIED, actual_cost=actual_cost, estimated_cost=estimated_cost),
            ...
            if action_budget.status in (ActionBudgetStatus.PAUSED, ActionBudgetStatus.CANCELLED):
                return EconomicBudgetDecision(EconomicBudgetAction.PAUSE, ("budget.action_budget_paused",), replace(resolved, status=EconomicBudgetStatus.PAUSED, estimated_cost=estimated_cost, actual_cost=actual_cost))
            if action_budget.status in (ActionBudgetStatus.EXHAUSTED, ActionBudgetStatus.COMPLETED):
                return EconomicBudgetDecision(EconomicBudgetAction.DENY, ("budget.exhausted",), replace(resolved, status=EconomicBudgetStatus.EXHAUSTED, exhausted=True, estimated_cost=estimated_cost, actual_cost=actual_cost))

        violations: list[str] = []
        if resolved.maximum_actual_cost_per_operation is not None and actual_cost > resolved.maximum_actual_cost_per_operation:
        ...
        if resolved.maximum_cost is not None:
            capacity = available_cost + reserved_cost if available_cost is not None else resolved.maximum_cost - used_cost
            if actual_cost > capacity:
                violations.append("budget.actual_cost_unavailable")
        excessive = bool(violations)
        if excessive:
            action = EconomicBudgetAction.REQUEST_APPROVAL if resolved.allow_overrun_with_approval else EconomicBudgetAction.PAUSE
            status = EconomicBudgetStatus.APPROVAL_REQUIRED if action is EconomicBudgetAction.REQUEST_APPROVAL else EconomicBudgetStatus.EXHAUSTED
            return EconomicBudgetDecision(
                action,
                tuple(violations),
                replace(resolved, status=status, actual_cost_excessive=True, approval_required=action is EconomicBudgetAction.REQUEST_APPROVAL, exhausted=status is EconomicBudgetStatus.EXHAUSTED, estimated_cost=estimated_cost, actual_cost=actual_cost),
            )
        return EconomicBudgetDecision(
            EconomicBudgetAction.ALLOW,
            ("budget.actual_cost_allowed",),
            replace(resolved, status=EconomicBudgetStatus.AVAILABLE, estimated_cost=estimated_cost, actual_cost=actual_cost),
        )

    @staticmethod
    def _blocked(resolved: ResolvedEconomicBudget, *codes: str, **flags: Any) -> EconomicBudgetDecision:
        action = EconomicBudgetAction.REQUEST_APPROVAL if resolved.allow_overrun_with_approval else EconomicBudgetAction.PAUSE
        status = (
        ...
        return EconomicBudgetDecision(
            action,
            tuple(codes),
            replace(resolved, status=status, approval_required=action is EconomicBudgetAction.REQUEST_APPROVAL, **flags),
        )

    @staticmethod
    def _most_restrictive_action(actions: Iterable[str]) -> str:
        rank = {"warn": 0, "reduce_scope": 1, "request_approval": 2, "pause": 3, "deny": 4}
