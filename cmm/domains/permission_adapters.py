...
def request_for_rule_permission(
    *, request_id: str, domain_id: str, actor_id: str, session_id: str, permission: str
) -> DomainPermissionRequest:
    try:
    ...
    *,
    ...
    for permission in dict.fromkeys(extra_capabilities):
        try:
            ...
            requirements.append(
                DomainOperationRequirementDecision(
    ...
    all_decisions = (
        root.effective_permissions.decision,
        *(item.decision for item in requirements),
    )
    reasons = (
        *root.effective_permissions.reasons,
        *(reason for item in requirements for reason in item.reasons),
    )
    provenance = tuple(
        item.source_id for item in root.effective_permissions.layer_evaluations
    )
    if not operation.enabled:
        return DomainOperationPermissionDecision(
            ...
            (*reasons, "operation_disabled"),
            requirement_decisions=tuple(requirements),
            provenance=provenance,
            effective_constraints=root.effective_permissions.effective_constraints,
        )
    ...
    decision = (
        ...
        if approvals or PermissionOutcome.APPROVAL_REQUIRED in all_decisions
...
def _node_decision(
    ...
    *,
    ...
    if branch is WorkflowNodePermissionBranch.OPERATION:
        ...
        result = evaluate_domain_operation(
            operation,
            resolver,
    ...
    if branch is WorkflowNodePermissionBranch.SUBWORKFLOW:
        ...
        decision = (
            PermissionOutcome.APPROVAL_REQUIRED if requirements else result.decision
        )
        return DomainWorkflowNodePermissionDecision(
            node.node_id,
            node.required,
    ...
    if branch is WorkflowNodePermissionBranch.APPROVAL:
        ...
        return DomainWorkflowNodePermissionDecision(
            ...
            (node.approval_gate or node.node_id,),
            (requirement,),
        )
    return DomainWorkflowNodePermissionDecision(
        node.node_id,
        ...
        ("node_has_no_permissioned_action",),
...
def evaluate_domain_workflow_requirements(
    ...
    *,
    ...
    root = resolver.resolve(
        ...
        supporting_domains=workflow.supporting_domain_ids,
        now=now,
    )
    requirements: list[DomainOperationRequirementDecision] = []
    for permission in workflow.required_permissions:
        ...
        effective = resolver.resolve(
            DomainPermissionRequest(
                f"{request_id}:permission:{permission}",
                action,
                workflow.domain_id,
                actor_id,
                session_id,
                ...
                workflow_version=workflow.version,
                purpose=workflow.purpose,
                sensitivity_level=workflow.sensitivity,
            ),
            now=now,
    ...
    for resource_id in workflow.required_resources:
        effective = resolver.resolve(
            DomainPermissionRequest(
                f"{request_id}:resource:{resource_id}",
                PermissionCapability.RESOURCE_READ,
                workflow.domain_id,
                actor_id,
                session_id,
    ...
    *,
    ...
    if not workflow.enabled or effective.decision is PermissionOutcome.DENY:
        reason = (
            ...
            else "workflow_permission_denied"
        )
        nodes = tuple(
            DomainWorkflowNodePermissionDecision(
                node.node_id,
                ...
                (*node.reasons, reason),
                node.provenance,
                node.approval_requirements,
            )
            for node in nodes
        )
    blocked = tuple(
        node.node_id for node in nodes if node.decision is PermissionOutcome.DENY
    )
    approval_nodes = tuple(
        node.node_id
        for node in nodes
        if node.decision is PermissionOutcome.APPROVAL_REQUIRED
    )
    allowed = tuple(
        node.node_id for node in nodes if node.decision is PermissionOutcome.ALLOW
    )
    ...
    required_blocked = any(
        node.required and node.decision is PermissionOutcome.DENY for node in nodes
    )
    requirement_denied = any(
        item.decision is PermissionOutcome.DENY for item in requirements
    )
    requirement_approval = any(
        item.decision is PermissionOutcome.APPROVAL_REQUIRED for item in requirements
    )
    decision = (
        PermissionOutcome.DENY
        if required_blocked or requirement_denied
    ...
    return DomainWorkflowPermissionDecision(
    ...
    *,
    ...
    for permission in rule.required_permissions:
        ...
        decisions.append(
            resolver.resolve(
                DomainPermissionRequest(
                    request_id, action, domain_id, actor_id, session_id
                )
            ).effective_permissions.decision
        )
    ...
    decision = (
        ...
        if PermissionOutcome.APPROVAL_REQUIRED in decisions
        else PermissionOutcome.ALLOW
    )
    return DomainRulePermissionDecision(
        rule.id,
        decision,
        required,
        blocked,
        blocked and not required,
        tuple(rule.required_permissions),
    )
