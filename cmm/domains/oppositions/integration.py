"""Phase 10.23 — Opposition Domain Integration.

Provides atomic, deterministic registration of the complete Opposition Domain
...
from cmm.domains.oppositions.permissions import build_oppositions_permission_policy
...
from cmm.domains.oppositions.rules import build_oppositions_rules
from cmm.domains.oppositions.workflows import (
    build_oppositions_workflow_definitions,
)


class OppositionsDomainIntegrationResult:
    """Compact result container for an Opposition Domain registration."""
...
def _validate_no_duplicate_workflows(workflow_registry, workflows) -> None:
    ...
    existing_ids = {
        ...
        for definition in workflow_registry.list_for_domain(str(OPPOSITIONS_DOMAIN_ID))
    }
    duplicates = tuple(
...
def _rollback(
    ...
    *,
    ...
    if rollback_errors:
        ...
        raise DomainError(
            "Opposition Domain registration failed and rollback was incomplete: "
            + "; ".join(rollback_errors)
        ) from original_error


def register_oppositions_domain(
    *,
    ...
    """Register the complete Opposition Domain atomically.
    ...
    is provided via ``operation_implementations``.
    ...
    *before* the first mutation, and snapshots of every registry are captured
    ...
    return OppositionsDomainIntegrationResult(
        definition=definition,
        profile=profile,
        resources=resources,
        rules=rules,
