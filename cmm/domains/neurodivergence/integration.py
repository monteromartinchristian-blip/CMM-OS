"""Phase 10.53 — Neurodivergence Domain Integration.

Provides atomic, deterministic registration of the complete Neurodivergence
...
class NeurodivergenceDomainIntegrationResult:
    """Compact result container for a Neurodivergence Domain registration."""
...
def _validate_no_duplicate_workflows(workflow_registry, workflows) -> None:
    ...
    existing_ids = {
        ...
        for definition in workflow_registry.list_for_domain(NEURODIVERGENCE_DOMAIN_ID)
    }
    duplicates = tuple(
...
def _rollback(
    ...
    *,
    ...
    if rollback_errors:
        ...
        raise DomainError(
            "Neurodivergence Domain registration failed and rollback was "
            "incomplete: " + "; ".join(rollback_errors)
        ) from original_error


def register_neurodivergence_domain(
    *,
    ...
    """Register the complete Neurodivergence Domain atomically.
    ...
    permission_policy = build_neurodivergence_permission_policy()
    ...
    return NeurodivergenceDomainIntegrationResult(
        definition=definition,
        profile=profile,
        resources=resources,
        rules=rules,
