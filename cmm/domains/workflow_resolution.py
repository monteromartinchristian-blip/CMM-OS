...
def resolve_domain_workflow(
    definition: DomainWorkflowDefinition, context: DomainWorkflowContext
) -> DomainWorkflowResolution:
    missing_permissions = tuple(
        sorted(set(definition.required_permissions) - context.available_permissions)
    )
    denied = tuple(
        sorted(set(definition.required_permissions) & context.denied_permissions)
    )
    missing_resources = tuple(
        sorted(set(definition.required_resources) - context.available_resources)
    )
    declared_domains = {
        definition.domain_id,
        *definition.supporting_domain_ids,
        *context.supporting_domain_ids,
    }
    unknown_domains = (
        declared_domains - set(context.known_domain_ids)
        if context.known_domain_ids
        else set()
    )
    unauthorized_domains = (
        (declared_domains & set(context.authorized_domain_ids)) ^ declared_domains
        if context.authorized_domain_ids
        else set()
    )
    unavailable_nodes = []
    ...
    if not definition.enabled:
        return DomainWorkflowResolution(
    ...
    if denied:
        return DomainWorkflowResolution(
            ...
            definition.version,
            definition.domain_id,
            WorkflowAvailabilityStatus.BLOCKED,
            ("permission.denied",),
            tuple(unavailable_nodes),
            missing_permissions,
            denied,
            missing_resources,
        )
    ...
    if set(definition.approval_gates) - context.approved_gates:
        return DomainWorkflowResolution(
            ...
            definition.version,
            definition.domain_id,
            WorkflowAvailabilityStatus.WAITING_FOR_APPROVAL,
            ("approval.pending",),
            (),
            missing_permissions,
            denied,
