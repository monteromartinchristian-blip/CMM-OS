"""Phase 10.29 — Life Plan Domain Integration.

Provides atomic, deterministic registration of the complete Life Plan Domain
...
from cmm.domains.life_plan.operations import build_life_plan_operation_definitions
...
from cmm.domains.life_plan.resources import build_life_plan_resource_definitions
from cmm.domains.life_plan.rules import build_life_plan_rules
from cmm.domains.life_plan.workflows import build_life_plan_workflow_definitions
from cmm.domains.operation_registry import validate_domain_operation_implementation


class LifePlanDomainIntegrationResult:
    """Compact result container for a Life Plan Domain registration."""
...
def _validate_no_duplicate_workflows(workflow_registry: Any, workflows: Any) -> None:
    ...
    existing_ids = {
        ...
        for definition in workflow_registry.list_for_domain(LIFE_PLAN_DOMAIN_ID)
    }
    duplicates = tuple(
...
def _rollback(
    ...
    *,
    ...
    if rollback_errors:
        ...
        raise DomainError(
            "Life Plan Domain registration failed and rollback was incomplete: "
            + "; ".join(rollback_errors)
        ) from original_error


def register_life_plan_domain(
    *,
    ...
    """Register the complete Life Plan Domain atomically."""
    ...
    operations = build_life_plan_operation_definitions()
    workflows = build_life_plan_workflow_definitions()
    permission_policy = build_life_plan_permission_policy()
    ...
    return LifePlanDomainIntegrationResult(
        definition=definition,
        profile=profile,
        resources=resources,
        rules=rules,
