"""Phase 10.50 – ``domain:sport`` privacy policy declaration.
...
__all__ = ["build_sport_privacy_policy"]


def build_sport_privacy_policy() -> DomainPrivacyPolicy:
    """Declare the approved ``domain:sport`` privacy default.

    Sport is conservative by default: current Sport resources include restricted
    and sensitive material, so the Domain declares local-only processing that
    resource and package privacy may only make stricter.
    """
