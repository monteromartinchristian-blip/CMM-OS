"""Phase 10.28 — Sport Domain Integration.

Provides atomic, deterministic registration of the complete Sport Domain
...
from cmm.domains.sport.permissions import build_sport_permission_policy
...
from cmm.domains.sport.rules import build_sport_rules
from cmm.domains.sport.workflows import (
    build_sport_workflow_definitions,
)


class SportDomainIntegrationResult:
    """Compact result container for a Sport Domain registration."""
...
def _validate_no_duplicate_workflows(workflow_registry: Any, workflows: Any) -> None:
    ...
    existing_ids = {
        ...
        for definition in workflow_registry.list_for_domain(SPORT_DOMAIN_ID)
    }
    duplicates = tuple(
...
def _rollback(
    ...
    *,
    ...
    if rollback_errors:
        ...
        raise DomainError(
            "Sport Domain registration failed and rollback was incomplete: "
            + "; ".join(rollback_errors)
        ) from original_error


def register_sport_domain(
    *,
    ...
    """Register the complete Sport Domain atomically."""
    ...
    operations = build_sport_operation_definitions()
    workflows = build_sport_workflow_definitions()
    permission_policy = build_sport_permission_policy()
    ...
    return SportDomainIntegrationResult(
        definition=definition,
        profile=profile,
        resources=resources,
        rules=rules,
