"""Phase 10.25 — Concerns Domain Integration.

Provides atomic, deterministic registration of the complete Concerns Domain
...
class ConcernsDomainIntegrationResult:
    """Compact result container for a Concerns Domain registration."""
...
def _validate_no_duplicate_workflows(workflow_registry, workflows) -> None:
    ...
    existing_ids = {
        ...
        for definition in workflow_registry.list_for_domain(CONCERNS_DOMAIN_ID)
    }
    duplicates = tuple(
    ...
    *,
    ...
    if permission_registry is not None:
        try:
            ...
            # not-found error is swallowed; any other exception propagates.
            pass
        else:
...
def _rollback(
    ...
    *,
    ...
    if rollback_errors:
        ...
        raise DomainError(
            "Concerns Domain registration failed and rollback was incomplete: "
            + "; ".join(rollback_errors)
        ) from original_error


def register_concerns_domain(
    *,
    ...
    """Register the complete Concerns Domain atomically.
    ...
    *before* the first mutation, and snapshots of every registry are captured
    ...
    return ConcernsDomainIntegrationResult(
        definition=definition,
        profile=profile,
        resources=resources,
        rules=rules,
