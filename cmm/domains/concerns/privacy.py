"""Phase 10.50 – ``domain:concerns`` privacy policy declaration.
...
__all__ = ["build_concerns_privacy_policy"]


def build_concerns_privacy_policy() -> DomainPrivacyPolicy:
    """Declare the approved ``domain:concerns`` privacy default.

    Concerns content is local-only by default and remains subject to canonical
