...
class DefaultDomainTraceReferenceValidator:
    ...
    def validate(
        ...
        if not isinstance(trace, DomainTrace):
            raise DomainTraceValidationError(
                "trace must be a DomainTrace", field="trace"
            )
        if not isinstance(inventory, DomainTraceReferenceInventory):
            raise DomainTraceValidationError(
                "inventory must be a DomainTraceReferenceInventory", field="inventory"
            )
        ...
        for label, check in (
            (
                "temporal",
                lambda: self._validate_temporal_integrity(
            ...
            (
                "references",
                lambda: self._validate_references(actual, inventory, codes, details),
            ),
            (
                "domain-results",
                lambda: self._validate_domain_results(trace, inventory, codes, details),
            ),
            (
                "cross-domain",
        ...
        trace_digest = (
            trace.digest
            if isinstance(getattr(trace, "digest", None), str)
            and re.fullmatch(r"[0-9a-f]{64}", trace.digest)
            else None
        )
        try:
        ...
        return DomainTraceValidationResult(
            ...
            missing_references=tuple(details["missing"]),
            unexpected_references=tuple(details["unexpected"]),
            duplicate_references=tuple(details["duplicate"]),
            reference_kind_mismatches=tuple(details["kind"]),
            reference_domain_mismatches=tuple(details["domain"]),
            invariant_failures=tuple(details["invariant"]),
    ...
    def _validate_final_contract(
        ...
        extra_fields = (
            set(vars(trace)) - set(DomainTrace.__dataclass_fields__)
    ...
    def _validate_structure(
        ...
        codes: list[DomainTraceValidationCode],
        details: dict[str, list[str]],
    ) -> tuple[DomainTraceReference, ...]:
        if (
            not isinstance(
                trace.primary_domain, type(inventory.expected_primary_domain)
            )
            or not isinstance(trace.supporting_domains, tuple)
            or not isinstance(trace.contributions, tuple)
        ):
            codes.append(DomainTraceValidationCode.INVALID_TRACE_CONTRACT)
        ...
        if (
            trace.primary_domain != inventory.expected_primary_domain
            or tuple(sorted(trace.supporting_domains, key=str)) != expected_supporting
        ):
        ...
        for selection_name in ("resolution_result_domains", "composition_domains"):
            ...
            if (
                selection.primary_domain != inventory.expected_primary_domain
                or selection.supporting_domains != expected_supporting
            ):
                codes.append(DomainTraceValidationCode.AUTHORITATIVE_DOMAIN_MISMATCH)
        ...
        for index, contribution in enumerate(trace.contributions):
            if (
                not hasattr(contribution, "domain_id")
                or not hasattr(contribution, "role")
                or not isinstance(contribution.references, tuple)
            ):
                codes.append(DomainTraceValidationCode.INVALID_TRACE_CONTRACT)
            ...
            expected_role = (
                DomainTraceRole.PRIMARY if index == 0 else DomainTraceRole.SUPPORTING
            )
        ...
        if tuple(domain_ids) != expected_domains:
            codes.append(DomainTraceValidationCode.AUTHORITATIVE_DOMAIN_MISMATCH)
        if not isinstance(trace.references, object) or not hasattr(
            trace.references, "all_references"
        ):
            codes.append(DomainTraceValidationCode.INVALID_TRACE_CONTRACT)
        else:
            try:
    ...
    def _validate_temporal_integrity(
        ...
        elif not isinstance(trace.duration_ms, int) or trace.duration_ms != int(
            (completed - started).total_seconds() * 1000
        ):
            codes.append(DomainTraceValidationCode.INVALID_DURATION)
        if not isinstance(trace.id, str) or not re.fullmatch(
            r"domain-trace:[0-9a-f]{24}", trace.id
        ):
            codes.append(DomainTraceValidationCode.INVALID_TRACE_ID)
        if not isinstance(trace.digest, str) or not re.fullmatch(
            r"[0-9a-f]{64}", trace.digest
        ):
            codes.append(DomainTraceValidationCode.INVALID_TRACE_DIGEST)
        else:
            try:
                if (
                    trace.digest != trace.calculate_digest()
                    or trace.id != trace.canonical_id
                ):
    ...
    def _validate_domain_results(
        ...
        contribution_result_items = [
            ...
            if isinstance(contribution, object) and hasattr(contribution, "references")
            for reference in contribution.references
            if isinstance(reference, DomainTraceReference)
            and reference.kind is DomainTraceReferenceKind.DOMAIN_RESULT
        ]
        contribution_results = set(contribution_result_items)
        pairings = {
            (item.result_id, item.domain_id)
            for item in trace.domain_results
            if hasattr(item, "result_id") and hasattr(item, "domain_id")
        }
        if (
            contribution_results != pairings
            or len(contribution_results) != len(contribution_result_items)
            or len(pairings) != len(trace.domain_results)
        ):
            codes.append(DomainTraceValidationCode.DOMAIN_RESULT_COVERAGE_MISMATCH)
            details["invariant"].append("domain-result-coverage")
        expected = {
            (item.result_id, item.domain_id, item.trace_id)
            for item in inventory.domain_results
        }
        actual = {
            (item.result_id, item.domain_id, item.trace_id)
            for item in trace.domain_results
    ...
    def _validate_cross_domain(
        ...
        actual = {
            (item.result_id, item.trace_id)
            for item in trace.references.cross_domain_results
        }
        expected = {
            (item.result_id, item.trace_id) for item in inventory.cross_domain_results
        }
        if actual != expected or any(
            item.trace_id is None for item in trace.references.cross_domain_results
        ):
        ...
        if any(
            inventory_by_id.get(item.result_id)
            is not DomainTraceReferenceKind.CROSS_DOMAIN_RESULT
            or inventory_by_id.get(item.trace_id)
            is not DomainTraceReferenceKind.CROSS_DOMAIN_TRACE
            for item in trace.references.cross_domain_results
        ):
            codes.append(DomainTraceValidationCode.CROSS_DOMAIN_PAIRING_MISMATCH)
...
def _contains_inline_content(value: Any) -> bool:
    if isinstance(value, str):
        ...
        ) or _contains_private_marker(value)
