"""Phase 10.21 — Relationships Domain Memory Integration.

Builds **proposal-only** memory views, proposals, and bindings using the real
Phase 10.18 contracts.  Relationships memory is never written autonomously:
sensitive relationship observations produce a ``DomainMemoryProposalSnapshot``
plus a ``DomainMemoryProposalBinding`` that link the proposal, view, digest,
trace, permissions, and approval references for the canonical approval path.
...
exposes no override — the confirmation invariant (see the profile's
``unconfirmed_sensitive_memory_persistence`` prohibition) cannot be silently
...
def build_relationships_memory_view_request(
    *,
    request_id: str,
    ...
    """Build a ``DomainMemoryViewRequest`` for ``domain:relationships``."""
    return DomainMemoryViewRequest(
        request_id=request_id,
        primary_domain=RELATIONSHIPS_DOMAIN_ID,
        trace_id=trace_id,
...
def build_relationships_memory_proposal(
    *,
    proposal_id: str,
    affected_reference_ids: tuple[str, ...] = (),
) -> DomainMemoryProposalSnapshot:
    """Build a relationship observation proposal (proposal-only, never applied).

    Sensitive Relationships memory is never written autonomously: every proposal
    is registered with ``requires_confirmation=True`` and the builder exposes no
    override, so the confirmation invariant cannot be silently disabled.
    """
...
def build_relationships_memory_view(
    *,
    request: DomainMemoryViewRequest,
    inventory,
) -> DomainMemoryView:
    """Resolve a proposal-only memory view for ``domain:relationships``.
...
def build_relationships_memory_binding(
    *,
    ...
    """Build a content-bound proposal binding for a relationship observation.
...
def validate_relationships_memory_binding(
    *,
