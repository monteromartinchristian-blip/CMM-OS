"""Phase 10.21 — Relationships Domain Integration.

Provides atomic, deterministic registration of the complete Relationships Domain
...
from cmm.domains.relationships.permissions import build_relationships_permission_policy
...
class RelationshipsDomainIntegrationResult:
    """Compact result container for a Relationships Domain registration."""
...
def _validate_no_duplicate_workflows(
    ...
    existing_ids = {
        ...
        for definition in workflow_registry.list_for_domain(RELATIONSHIPS_DOMAIN_ID)
    }
    duplicates = tuple(
...
def _rollback(
    ...
    *,
    ...
    if rollback_errors:
        ...
        raise DomainError(
            "Relationships Domain registration failed and rollback was incomplete: "
            + "; ".join(rollback_errors)
        ) from original_error


def register_relationships_domain(
    *,
    ...
    """Register the complete Relationships Domain atomically.
    ...
    resources = build_relationships_resource_definitions()
    rules = build_relationships_rules()
    operations = build_relationships_operation_definitions()
    workflows = build_relationships_workflow_definitions()
    permission_policy = build_relationships_permission_policy()
    ...
    return RelationshipsDomainIntegrationResult(
        definition=definition,
        profile=profile,
        resources=resources,
        rules=rules,
