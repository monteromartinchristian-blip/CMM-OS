"""Phase 10.21 — Relationships Domain Trace.

Relationships Domain composes caller-supplied typed references into the canonical
...
only when the caller supplies them.  No provenance or relational evidence is
invented.
"""
...
def build_relationships_trace_reference(
    *,
    ref_id: str,
    kind: DomainTraceReferenceKind,
    domain_id: str = RELATIONSHIPS_DOMAIN_ID,
) -> DomainTraceReference:
    """Build a domain-scoped ``DomainTraceReference`` owned by Relationships Domain.
...
def build_relationships_trace_contribution(
    *,
    domain_result_id: str,
    references: tuple[DomainTraceReference, ...] = (),
    domain_id: str = RELATIONSHIPS_DOMAIN_ID,
) -> DomainTraceContribution:
    """Build a PRIMARY ``DomainTraceContribution`` for Relationships Domain.
...
def assemble_relationships_trace(
    *,
    request_id: str,
    ...
    """Assemble a final reference-only ``DomainTrace`` for Relationships Domain.
    ...
    contribution = build_relationships_trace_contribution(
...
def validate_relationships_trace(
    *,
    trace: DomainTrace,
