    ...
    def evaluate(self, context: ReasoningRuleContext) -> ReasoningRuleResult:
        if self.behavior == "distinguish":
            if not context.knowledge_items:
                return _result(
                    ...
                    message="No knowledge items require epistemic classification.",
                )
            return _result(
                self.definition,
                context,
                ReasoningRuleResultStatus.APPLIED,
                code="EPISTEMIC_KINDS_PRESERVED",
                message="Existing epistemic kinds were preserved.",
            )
        ...
        if self.behavior == "provenance":
            if not context.knowledge_items:
                return _result(
                    ...
                    ReasoningRuleResultStatus.NOT_APPLICABLE,
                    code="RULE_NOT_APPLICABLE",
                    message="No knowledge items require provenance inspection.",
                )
            missing = tuple(
                item
                for item in context.knowledge_items
                if not item.evidence and item.resource_id is None
            )
            findings = tuple(
                ReasoningFinding(
                    code="PROVENANCE_MISSING",
                    message="A knowledge item lacks evidence provenance.",
            ...
            gaps = tuple(
                ReasoningGap(
                    code="PROVENANCE_GAP",
                    message="Evidence provenance is required before materialization.",
                    severity=ReasoningSeverity.WARNING,
                    rule_id=self.definition.id,
                    references=(item.id,),
                )
        ...
        if self.behavior == "sensitivity":
            if (
                ...
                and "sensitivity.read" not in context.effective_permissions
            ):
                ...
                return _result(
                    ...
                    findings=(finding,),
                    code="RULE_BLOCKED",
                    message="Sensitivity permission is missing.",
                )
            return _result(
                self.definition,
                context,
                ReasoningRuleResultStatus.APPLIED,
                code="SENSITIVITY_RESPECTED",
                message="No sensitivity boundary was weakened.",
            )
        ...
        if self.behavior == "permissions":
            ...
            if missing:
                ...
                return _result(
                    ...
                    findings=(finding,),
                    code="RULE_BLOCKED",
                    message="Inference permission is missing.",
                )
            return _result(
                self.definition,
                context,
                ReasoningRuleResultStatus.APPLIED,
                code="INFERENCE_PERMISSIONS_RESPECTED",
                message="No unauthorized inference was requested.",
            )

        if self.behavior == "health_red_flags":
            health = context.metadata.get("health")
            present = (
                isinstance(health, Mapping) and health.get("red_flags_present") is True
            )
            if not present:
                return _result(
                    self.definition,
                    ...
                    ReasoningRuleResultStatus.NOT_APPLICABLE,
                    code="RULE_NOT_APPLICABLE",
                    message="No structured health red-flag signal was supplied.",
                )
        ...
        if self.behavior == "deadline":
            ...
            if not isinstance(academic, Mapping) or "deadline" not in academic:
                gap = ReasoningGap(
                    code="DEADLINE_INFORMATION_GAP",
                    message="No structured deadline was supplied.",
                    severity=ReasoningSeverity.WARNING,
                    rule_id=self.definition.id,
                    domain_id=self.definition.domain_id,
                )
                return _result(
                    ...
                    gaps=(gap,),
                    code="GAP_RECORDED",
                    message="A deadline information gap was recorded.",
                )
            return _result(
                self.definition,
                context,
                ...
                message="A structured deadline is present.",
            )
        ...
        return _result(
            ...
            message="The future Domain Pack semantics are not activated in Phase 10.12.",
        )


def _global(
    rule_id: str, name: str, category: str, priority: int
) -> ReasoningRuleDefinition:
    return ReasoningRuleDefinition(
        ...
        risk_level="low",
        deterministic=True,
        description="Conservative structural reasoning rule.",
        metadata={"phase": "10.12"},
    )
...
def build_initial_reasoning_rule_catalog() -> InMemoryReasoningRuleRegistry:
    ...
    entries = (
        ...
        (
            _global(
                "global.preserve_provenance", "PreserveProvenance", "epistemic", 990
            ),
            "provenance",
        ),
        (
            _global(
                "security.respect_sensitivity", "RespectSensitivity", "safety", 980
            ),
            "sensitivity",
        ),
        (
        ...
        (
            _domain(
                "health.medication_temporal_relationship",
                "MedicationTemporalRelationship",
                "domain:health",
                "temporality",
                710,
            ),
        ...
        (
            _domain(
                "relationships.fact_interpretation",
                "RelationshipFactInterpretation",
                "domain:relationship",
                "epistemic",
                700,
            ),
        ...
        (
            _domain(
                "project.code_documentation_consistency",
                "ProjectCodeDocumentationConsistency",
                "domain:project",
                "consistency",
                690,
            ),
