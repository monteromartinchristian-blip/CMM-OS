"""Phase 10.22 — University Domain Trace.

University Domain composes caller-supplied typed references into the canonical
...
only when the caller supplies them.  No provenance or academic evidence is
invented.
"""
...
def build_university_trace_reference(
    *,
    ref_id: str,
    kind: DomainTraceReferenceKind,
    domain_id: str = UNIVERSITY_DOMAIN_ID,
) -> DomainTraceReference:
    """Build a domain-scoped ``DomainTraceReference`` owned by University Domain.
...
def build_university_trace_contribution(
    *,
    domain_result_id: str,
    references: tuple[DomainTraceReference, ...] = (),
    domain_id: str = UNIVERSITY_DOMAIN_ID,
) -> DomainTraceContribution:
    """Build a PRIMARY ``DomainTraceContribution`` for University Domain.
...
def assemble_university_trace(
    *,
    ...
    """Assemble a final reference-only ``DomainTrace`` for University Domain.
    ...
    contribution = build_university_trace_contribution(
