"""Phase 10.22 — University Domain Integration.

Provides atomic, deterministic registration of the complete University Domain
...
from cmm.domains.university.rules import build_university_rules
from cmm.domains.university.workflows import (
    build_university_workflow_definitions,
)


class UniversityDomainIntegrationResult:
    """Compact result container for a University Domain registration."""
...
def _validate_no_duplicate_workflows(
    ...
    existing_ids = {
        ...
        for definition in workflow_registry.list_for_domain(str(UNIVERSITY_DOMAIN_ID))
    }
    duplicates = tuple(
...
def _rollback(
    ...
    *,
    ...
    if rollback_errors:
        ...
        raise DomainError(
            "University Domain registration failed and rollback was incomplete: "
            + "; ".join(rollback_errors)
        ) from original_error


def register_university_domain(
    *,
    ...
    """Register the complete University Domain atomically.
    ...
    resources = build_university_resource_definitions()
    rules = build_university_rules()
    operations = build_university_operation_definitions()
    workflows = build_university_workflow_definitions()
    permission_policy = build_university_permission_policy()
    ...
    return UniversityDomainIntegrationResult(
        definition=definition,
        profile=profile,
        resources=resources,
        rules=rules,
