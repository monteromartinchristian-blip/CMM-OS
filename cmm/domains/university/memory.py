"""Phase 10.22 — University Domain Memory Integration.

Builds **proposal-only** memory views, proposals, and bindings using the real
Phase 10.18 contracts.  University memory is never written autonomously:
academic observations produce a ``DomainMemoryProposalSnapshot`` plus a
...
exposes no override — the confirmation invariant cannot be silently disabled.
Personal memory (``university.memory_entry``) never overrides Academic State.
"""
...
def build_university_memory_view_request(
    *,
    request_id: str,
    ...
    """Build a ``DomainMemoryViewRequest`` for ``domain:university``."""
    return DomainMemoryViewRequest(
        request_id=request_id,
        primary_domain=UNIVERSITY_DOMAIN_ID,
        trace_id=trace_id,
...
def build_university_memory_proposal(
    *,
    proposal_id: str,
    affected_reference_ids: tuple[str, ...] = (),
) -> DomainMemoryProposalSnapshot:
    """Build an academic observation proposal (proposal-only, never applied).

    Sensitive University memory is never written autonomously: every proposal
    is registered with ``requires_confirmation=True`` and the builder exposes
    no override, so the confirmation invariant cannot be silently disabled.
    """
...
def build_university_memory_view(
    *,
    request: DomainMemoryViewRequest,
    inventory,
) -> DomainMemoryView:
    """Resolve a proposal-only memory view for ``domain:university``.
...
def build_university_memory_binding(
    *,
    ...
    """Build a content-bound proposal binding for an academic observation.
...
def validate_university_memory_binding(
    *,
