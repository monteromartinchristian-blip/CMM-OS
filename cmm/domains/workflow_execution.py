...
class DomainWorkflowExecutor:
    ...
    def execute(
        ...
        if self._permission_gate is not None:
            ...
            if gate_result.requires_approval or approval_pending:
                self._definitions[(definition.workflow_id, definition.version)] = (
                    definition
                )
                now = (
                    self._clock()
        ...
        def adapter(node, run):
            ...
            if self._operation_adapter is not None:
                ...
                return value
            if (
                node.node_type.value == "request_approval"
                and node.approval_gate not in context.approved_gates
            ):
                approval = ApprovalRequest(
                    self._id_factory(),
                    definition.workflow_id,
        ...
        result: WorkflowExecutionResult = WorkflowEngine(
            definition.to_common(),
    ...
    def execute_result(
        ...
        return DomainWorkflowResult(
            WorkflowResult(
                ...
                wait_request=execution.wait_request,
                attempts=execution.attempts,
                defined_node_ids=tuple(node.node_id for node in definition.nodes),
    ...
    def resume(
        ...
        *,
        ...
        permission_approval_request_id: str | None = None,
        permission_approval_request_ids: Mapping[str, str] | None = None,
    ) -> DomainWorkflowRun:
        ...
        if state is not None:
            ...
            if permission_approval_request_ids:
                state["approval_request_ids"].update(permission_approval_request_ids)
            if (
        ...
        result = self._engine_for_run(run).resume(
            run.common_run,
        ...
        return DomainWorkflowRun(
            result.run, run.primary_domain_id, run.provenance, result
        )

    def _engine_for_run(self, run: DomainWorkflowRun) -> WorkflowEngine:
        definition = self._definitions.get(
            (run.common_run.workflow_id, run.common_run.workflow_version)
        )
