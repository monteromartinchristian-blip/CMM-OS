"""Phase 10.50 – ``domain:health`` privacy policy declaration.
...
__all__ = ["build_health_privacy_policy"]


def build_health_privacy_policy() -> DomainPrivacyPolicy:
    """Declare the approved ``domain:health`` privacy default.

    Health content is local-only by default and remains subject to canonical
