"""Phase 10.20 — Health Domain Integration.

Provides atomic, deterministic registration of the complete Health Domain
...
from cmm.domains.health.operations import build_health_operation_definitions
...
from cmm.domains.health.resources import build_health_resource_definitions
from cmm.domains.health.rules import build_health_rules
from cmm.domains.health.workflows import build_health_workflow_definitions
from cmm.domains.operation_registry import validate_domain_operation_implementation


class HealthDomainIntegrationResult:
    """Compact result container for a Health Domain registration."""
...
def _validate_no_duplicate_operations(
    ...
    nested common registry.  Two layers are checked, both deterministically
    before the first mutation (see General Domain for the rationale).
    """
    existing_ids = {
...
def _validate_no_duplicate_workflows(
    ...
    nested common registry, before any mutation (see General Domain).
    """
    from cmm.domains.health.definition import HEALTH_DOMAIN_ID

    existing_ids = {
        definition.workflow_id
        for definition in workflow_registry.list_for_domain(HEALTH_DOMAIN_ID)
    }
    duplicates = tuple(
...
def _rollback(
    ...
    *,
    ...
    if rollback_errors:
        ...
        raise DomainError(
            "Health Domain registration failed and rollback was incomplete: "
...
def register_health_domain(
    *,
    ...
    """Register the complete Health Domain atomically.
    ...
    resources = build_health_resource_definitions()
    rules = build_health_rules()
    operations = build_health_operation_definitions()
    workflows = build_health_workflow_definitions()
    permission_policy = build_health_permission_policy()
    ...
    return HealthDomainIntegrationResult(
        definition=definition,
        profile=profile,
        resources=resources,
        rules=rules,
...
__all__ = ["HealthDomainIntegrationResult", "register_health_domain"]
