"""Phase 10.26 — Languages Domain Integration.

Provides atomic, deterministic registration of the complete Languages Domain
...
class LanguagesDomainIntegrationResult:
    """Compact result container for a Languages Domain registration."""
...
def _validate_no_duplicate_workflows(workflow_registry: Any, workflows: Any) -> None:
    ...
    existing_ids = {
        ...
        for definition in workflow_registry.list_for_domain(LANGUAGES_DOMAIN_ID)
    }
    duplicates = tuple(
...
def _rollback(
    ...
    *,
    ...
    if rollback_errors:
        ...
        raise DomainError(
            "Languages Domain registration failed and rollback was incomplete: "
            + "; ".join(rollback_errors)
        ) from original_error


def register_languages_domain(
    *,
    ...
    """Register the complete Languages Domain atomically."""
    definition = build_languages_domain_definition()
    profile = build_languages_profile()
    resources = build_languages_resource_definitions()
    ...
    permission_policy = build_languages_permission_policy()
