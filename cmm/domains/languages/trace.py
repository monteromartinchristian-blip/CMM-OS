"""Phase 10.26 — Languages Domain Trace Integration.

Composes caller-supplied typed references into the canonical Phase 10.17 trace contracts.
Traces reference — never duplicate — rule/source/operation/permission references,
and no private chain-of-thought is persisted.
"""
...
def build_languages_trace_reference(
    *,
    ref_id: str,
    kind: DomainTraceReferenceKind,
    domain_id: str = LANGUAGES_DOMAIN_ID,
) -> DomainTraceReference:
    """Build a domain-scoped ``DomainTraceReference`` owned by Languages Domain."""
...
def build_languages_trace_contribution(
    *,
    domain_result_id: str,
    references: tuple[DomainTraceReference, ...] = (),
    domain_id: str = LANGUAGES_DOMAIN_ID,
) -> DomainTraceContribution:
    """Build a PRIMARY ``DomainTraceContribution`` for Languages Domain."""
    all_references = [
        DomainTraceReference(
...
def assemble_languages_trace(
    *,
    ...
    if contributions:
        primary = build_languages_trace_contribution(
