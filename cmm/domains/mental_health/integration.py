"""Phase 10.52 — Mental Health Domain Integration.

Provides atomic, deterministic registration of the complete Mental Health
Domain across all relevant registries.  Registration is **validation-first**
and **rollback-capable**: all inputs are validated against every registry
before the first mutation, and snapshots of every registry are captured before
any mutation.  If any registration raises after a mutation, all registries are
restored to their exact prior state via public ``restore_state()`` APIs.

No new transaction manager or registry abstraction is introduced: this mirrors
the canonical neighboring first-party registration pattern.

Operations are registered as **UNAVAILABLE** (fail-closed) unless a real
implementation is explicitly injected.  Declaration never means availability.
"""

from __future__ import annotations

from cmm.domains.errors import DomainPermissionRegistryError
from cmm.domains.mental_health.definition import build_mental_health_domain_definition
...
class MentalHealthDomainIntegrationResult:
    """Compact result container for a Mental Health Domain registration."""
...
def _validate_operation_implementations(
    ...
    """Validate every provided implementation against its declared operation.

    Operations without an implementation are allowed and register as
    **UNAVAILABLE** (fail-closed).  Validation runs before the first mutation.
    """
...
def _validate_no_duplicate_operations(operation_registry, operations) -> None:
    """Validate no operation key is already registered, before any mutation."""
    existing_ids = {
...
def _validate_no_duplicate_workflows(workflow_registry, workflows) -> None:
    """Validate no workflow key is already registered, before any mutation."""
    from cmm.domains.mental_health.catalog import MENTAL_HEALTH_DOMAIN_ID

    existing_ids = {
        definition.workflow_id
        for definition in workflow_registry.list_for_domain(MENTAL_HEALTH_DOMAIN_ID)
    }
    duplicates = tuple(
...
def _validate_all(
    *,
    ...
    if resource_registry is not None:
        existing_ids = {resource.id for resource in resource_registry.list_all()}
        duplicates = tuple(
...
def _rollback(
    ...
    *,
    ...
    If any restore fails, raise a composite sanitized error instead of leaving
    a partially registered Domain behind.
    """
    ...
    if rollback_errors:
        ...
        raise DomainError(
            "Mental Health Domain registration failed and rollback was "
            "incomplete: " + "; ".join(rollback_errors)
        ) from original_error


def register_mental_health_domain(
    *,
    ...
    """Register the complete Mental Health Domain atomically.

    Each registry is optional; ``None`` skips that registration.  Operations
    are registered as **UNAVAILABLE** (fail-closed) unless an implementation is
    explicitly injected via ``operation_implementations``.

    Registration is **atomic**: all inputs are validated against every registry
    before the first mutation, and snapshots of every registry are captured
    before any mutation.  If any registration raises after a mutation, all
    registries are restored to their exact prior state.

    No global mutable state is modified.
    """
    definition = build_mental_health_domain_definition()
    profile = build_mental_health_profile()
    resources = build_mental_health_resource_definitions()
    rules = build_mental_health_rules()
    operations = build_mental_health_operation_definitions()
    workflows = build_mental_health_workflow_definitions()
    permission_policy = build_mental_health_permission_policy()
    ...
    return MentalHealthDomainIntegrationResult(
        definition=definition,
        profile=profile,
        resources=resources,
        rules=rules,
