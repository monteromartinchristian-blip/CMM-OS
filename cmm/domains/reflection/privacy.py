"""Phase 10.50 – ``domain:reflection`` privacy policy declaration.
...
__all__ = ["build_reflection_privacy_policy"]


def build_reflection_privacy_policy() -> DomainPrivacyPolicy:
    """Declare the approved ``domain:reflection`` privacy default.

    Reflection content is local-only by default and remains subject to canonical
