"""Phase 10.24 — Reflection Domain Integration.

Provides atomic, deterministic registration of the complete Reflection Domain
...
class ReflectionDomainIntegrationResult:
    """Compact result container for a Reflection Domain registration."""
...
def _validate_no_duplicate_workflows(workflow_registry, workflows) -> None:
    ...
    existing_ids = {
        ...
        for definition in workflow_registry.list_for_domain(str(REFLECTION_DOMAIN_ID))
    }
    duplicates = tuple(
...
def _rollback(
    ...
    *,
    ...
    if rollback_errors:
        ...
        raise DomainError(
            "Reflection Domain registration failed and rollback was incomplete: "
            + "; ".join(rollback_errors)
        ) from original_error


def register_reflection_domain(
    *,
    ...
    """Register the complete Reflection Domain atomically.
    ...
    *before* the first mutation, and snapshots of every registry are captured
    ...
    return ReflectionDomainIntegrationResult(
        definition=definition,
        profile=profile,
        resources=resources,
        rules=rules,
