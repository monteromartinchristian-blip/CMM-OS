"""Phase 10.19 — Canonical General Domain Bootstrap.

Provides the official, recommended factory for constructing the standard
domain registries with General Domain integrated.  This is the canonical
composition path for callers that need a complete, discoverable General
Domain without manual registry wiring.

The bootstrap builds the standard registries, registers General Domain
atomically, and exposes the standard ``DefaultDomainResolver`` configured
with ``domain:general`` as its fallback domain.
...
class GeneralDomainBootstrap:
    """The standard registries with General Domain fully integrated."""
...
def build_standard_general_domain_bootstrap(
    *,
    operation_implementations: dict | None = None,
) -> GeneralDomainBootstrap:
    """Build the standard registries with General Domain integrated.

    This is the canonical, recommended composition path.  It creates fresh
    registries, registers the complete General Domain atomically, and
