...
def _convert_contract(exc: ReasoningRuleContractError) -> DomainRuleContractError:
    return DomainRuleContractError(
...
def _from_dict(cls: type, data: Mapping[str, Any], required: set[str]) -> Any:
    if not isinstance(data, Mapping):
        raise DomainRuleSerializationError(
            f"{cls.__name__}.from_dict requires a mapping", field="data"
        )
    try:
...
def _nested(value: Any, cls: type, field_name: str) -> tuple[Any, ...]:
    ...
    for index, item in enumerate(value):
        if isinstance(item, cls):
            ...
            raise DomainRuleContractError(
                f"{field_name}[{index}] must be a {cls.__name__}",
...
class DomainReasoningRuleDefinition(ReasoningRuleDefinition):
    def __post_init__(self) -> None:
        ...
        if self.domain_id is None or self.scope.value != "domain":
            raise DomainRuleContractError(
                "DomainReasoningRuleDefinition requires domain scope and domain_id",
                field="domain_id",
            )
...
class DomainRuleResult(ReasoningRuleResult):
    def __post_init__(self) -> None:
        ...
        if self.domain_id is None:
            raise DomainRuleContractError(
                "DomainRuleResult requires domain_id", field="domain_id"
            )
    ...
    def from_dict(cls, data: Mapping[str, Any]) -> DomainRuleResult:
        if not isinstance(data, Mapping):
            raise DomainRuleSerializationError(
                "DomainRuleResult.from_dict requires a mapping", field="data"
            )
        try:
        ...
        return _construct(
            cls, {name: getattr(common, name) for name in cls.__dataclass_fields__}
        )
...
class DomainRuleSelectionPolicy:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.include_optional, bool):
            raise DomainRuleContractError(
                "include_optional must be a strict boolean", field="include_optional"
            )
        if not isinstance(self.include_requested, bool):
            raise DomainRuleContractError(
                "include_requested must be a strict boolean", field="include_requested"
            )
        try:
            object.__setattr__(
                self,
                "denied_permissions",
                _str_tuple(self.denied_permissions, "denied_permissions"),
            )
            object.__setattr__(self, "metadata", _json_freeze(self.metadata))
            object.__setattr__(
                self,
                "contract_version",
                _semver(self.contract_version, "contract_version"),
            )
        except ReasoningRuleContractError as exc:
            raise _convert_contract(exc) from exc
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "include_requested": self.include_requested,
...
class DomainRuleExecutionPolicy:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.stop_on_required_failure, bool):
            raise DomainRuleContractError(
                "stop_on_required_failure must be a strict boolean",
                field="stop_on_required_failure",
            )
        if isinstance(self.aggregate_confidence_limit, bool) or not isinstance(
            self.aggregate_confidence_limit, (int, float)
        ):
            raise DomainRuleContractError(
                "aggregate_confidence_limit must be numeric",
                field="aggregate_confidence_limit",
            )
        ...
        if (
            ...
            raise DomainRuleContractError(
                "aggregate_confidence_limit must be finite and in (0, 1]",
                field="aggregate_confidence_limit",
            )
...
class DomainRuleSourceRecord:
    ...
    def __post_init__(self) -> None:
        try:
            object.__setattr__(
                self, "source", _enum(self.source, DomainRuleSource, "source")
            )
            object.__setattr__(
                self, "reference", _non_empty(self.reference, "reference")
            )
            object.__setattr__(
                self,
                "domain_id",
                _domain_id(self.domain_id, "domain_id", optional=True),
            )
            if self.profile_name is not None:
                object.__setattr__(
                    self, "profile_name", _non_empty(self.profile_name, "profile_name")
                )
            object.__setattr__(self, "metadata", _json_freeze(self.metadata))
        except ReasoningRuleContractError as exc:
            raise _convert_contract(exc) from exc
        if not isinstance(self.required, bool):
            raise DomainRuleContractError(
                "required must be a strict boolean", field="required"
            )
        if (
            ...
            raise DomainRuleContractError(
                "precedence must be a non-negative integer", field="precedence"
            )
...
class DomainRuleSelectionDecision:
    ...
    def __post_init__(self) -> None:
        try:
            object.__setattr__(
                self, "code", _enum(self.code, DomainRuleSelectionDecisionCode, "code")
            )
            ...
            object.__setattr__(
                self,
                "missing_permissions",
                _str_tuple(self.missing_permissions, "missing_permissions"),
            )
            object.__setattr__(self, "metadata", _json_freeze(self.metadata))
        except ReasoningRuleContractError as exc:
            raise _convert_contract(exc) from exc
        if not isinstance(self.included, bool):
            raise DomainRuleContractError(
                "included must be a strict boolean", field="included"
            )
        object.__setattr__(
            self, "sources", _nested(self.sources, DomainRuleSourceRecord, "sources")
        )
    ...
    def __post_init__(self) -> None:
        try:
            ...
            object.__setattr__(
                self,
                "severity",
                _enum(self.severity, DomainRuleConflictSeverity, "severity"),
            )
            object.__setattr__(
                self,
                "missing_permissions",
...
class SelectedReasoningRule:
    ...
    def __post_init__(self) -> None:
        if isinstance(self.definition, Mapping):
            try:
                object.__setattr__(
                    self,
                    "definition",
                    ReasoningRuleDefinition.from_dict(self.definition),
                )
            except ReasoningRuleSerializationError as exc:
        ...
        if not isinstance(self.definition, ReasoningRuleDefinition):
            raise DomainRuleContractError(
                "definition must be a ReasoningRuleDefinition", field="definition"
            )
        object.__setattr__(
            self, "sources", _nested(self.sources, DomainRuleSourceRecord, "sources")
        )
        ...
        try:
            object.__setattr__(
                self, "group", _enum(self.group, DomainRuleSource, "group")
            )
        except ReasoningRuleContractError as exc:
            raise _convert_contract(exc) from exc
        if not isinstance(self.required, bool):
...
class DomainRuleExecutionPlan:
    ...
    def __post_init__(self) -> None:
        try:
            ...
            object.__setattr__(
                self, "status", _enum(self.status, DomainRuleSelectionStatus, "status")
            )
            object.__setattr__(
            ...
            object.__setattr__(
                ...
                "omitted_rule_ids",
                tuple(
                    _rule_id(x, "omitted_rule_ids")
                    for x in _str_tuple(self.omitted_rule_ids, "omitted_rule_ids")
                ),
            )
            object.__setattr__(
                self,
                "blocked_rule_ids",
                tuple(
                    _rule_id(x, "blocked_rule_ids")
                    for x in _str_tuple(self.blocked_rule_ids, "blocked_rule_ids")
                ),
            )
            object.__setattr__(
                self,
            ...
            object.__setattr__(
                ...
                _str_tuple(self.contributing_profiles, "contributing_profiles"),
            )
            object.__setattr__(
                self,
                "contributing_domains",
                tuple(
                    _domain_id(x, "contributing_domains")
                    for x in _str_tuple(
                        self.contributing_domains, "contributing_domains"
                    )
                ),
            )
            object.__setattr__(self, "metadata", _json_freeze(self.metadata))
        ...
        object.__setattr__(
            self,
            "selected_rules",
            _nested(self.selected_rules, SelectedReasoningRule, "selected_rules"),
        )
        object.__setattr__(
            self,
            "decisions",
            _nested(self.decisions, DomainRuleSelectionDecision, "decisions"),
        )
        object.__setattr__(
            self,
            "conflicts",
            _nested(self.conflicts, DomainRuleSelectionConflict, "conflicts"),
        )
        keys = [(x.definition.id, x.definition.version) for x in self.selected_rules]
        if len(keys) != len(set(keys)):
            raise DomainRuleContractError(
                "selected_rules must be unique by id/version", field="selected_rules"
            )
        if (
            self.status is DomainRuleSelectionStatus.BLOCKED
            and not self.conflicts
            and not self.blocked_rule_ids
        ):
            raise DomainRuleContractError(
                "blocked plan requires conflicts or blocked rules", field="status"
            )
        if self.status is DomainRuleSelectionStatus.READY and any(
            x.blocking for x in self.conflicts
        ):
            raise DomainRuleContractError(
                "ready plan cannot contain blocking conflicts", field="status"
            )
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "omitted_rule_ids": list(self.omitted_rule_ids),
    ...
    def from_dict(cls, data: Mapping[str, Any]) -> DomainRuleExecutionPlan:
        if not isinstance(data, Mapping):
            raise DomainRuleSerializationError(
                "DomainRuleExecutionPlan.from_dict requires a mapping", field="data"
            )
        values = dict(data)
        try:
            _reject_unknown(data, set(cls.__dataclass_fields__), cls.__name__)
            ...
            raise DomainRuleSerializationError(
                getattr(exc, "message", "invalid plan"),
                field=getattr(exc, "field", None),
            ) from exc
...
class DomainRuleExecutionResult:
    ...
    def __post_init__(self) -> None:
        try:
            ...
            object.__setattr__(
                self, "status", _enum(self.status, DomainRuleExecutionStatus, "status")
            )
            object.__setattr__(
                self, "started_at", _aware(self.started_at, "started_at")
            )
            object.__setattr__(
            ...
            for name in (
                ...
                object.__setattr__(
                    ...
                    name,
                    tuple(
                        _rule_id(x, name) for x in _str_tuple(getattr(self, name), name)
                    ),
                )
            object.__setattr__(self, "metadata", _json_freeze(self.metadata))
            object.__setattr__(
        ...
        object.__setattr__(
            self,
            "rule_results",
            _nested(self.rule_results, ReasoningRuleResult, "rule_results"),
        )
        object.__setattr__(
            self, "findings", _nested(self.findings, ReasoningFinding, "findings")
        )
        object.__setattr__(self, "gaps", _nested(self.gaps, ReasoningGap, "gaps"))
        object.__setattr__(
            self,
            "recommendations",
            _nested(self.recommendations, ReasoningRecommendation, "recommendations"),
        )
        object.__setattr__(
            self,
            "escalations",
            _nested(self.escalations, ReasoningEscalation, "escalations"),
        )
        object.__setattr__(
            self,
            "trace_entries",
            _nested(self.trace_entries, ReasoningRuleTraceEntry, "trace_entries"),
        )
        object.__setattr__(
            self,
            "decisions",
        ...
        if (
            ...
            or not all(
                isinstance(item, KnowledgeItem) for item in self.produced_knowledge
            )
        ):
            raise DomainRuleContractError(
    ...
    def from_dict(cls, data: Mapping[str, Any]) -> DomainRuleExecutionResult:
        if not isinstance(data, Mapping):
            raise DomainRuleSerializationError(
                "DomainRuleExecutionResult.from_dict requires a mapping", field="data"
            )
        values = dict(data)
        try:
            _reject_unknown(data, set(cls.__dataclass_fields__), cls.__name__)
