"""Phase 10.30 — Project Domain Trace Integration.
...
def build_project_trace_reference(
    *,
    ref_id: str,
    kind: DomainTraceReferenceKind,
    domain_id: str = PROJECT_DOMAIN_ID,
) -> DomainTraceReference:
    """Build a domain-scoped ``DomainTraceReference`` owned by Project Domain."""
...
def build_project_trace_contribution(
    *,
    domain_result_id: str,
    references: tuple[DomainTraceReference, ...] = (),
    domain_id: str = PROJECT_DOMAIN_ID,
) -> DomainTraceContribution:
    """Build a PRIMARY ``DomainTraceContribution`` for Project Domain."""
    all_references = [
        DomainTraceReference(
...
def assemble_project_trace(
    *,
    ...
    """Assemble a final reference-only ``DomainTrace`` for Project Domain."""
    now = started_at or datetime.now(timezone.utc)
    end = completed_at or now
    if contributions:
        primary = build_project_trace_contribution(
