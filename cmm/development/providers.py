...
from kernel.llm.provider_registry import get_provider_spec, has_provider
...
class PlanningProvider(Protocol):
    def generate_plan(
        self, goal: str, context: ProjectContext
    ) -> DevelopmentPlan | Mapping[str, Any]:
...
class DeterministicPlanningProvider:
        ...
        if stripped.startswith("{"):
            try:
                ...
                raise PlanningProviderError(
                    f"Invalid JSON development goal: {error}"
                ) from error
            if not isinstance(payload, Mapping):
                raise PlanningProviderError(
                    "A JSON development goal must contain a plan object."
                )
            return payload
    ...
    def generate_correction(
        self, goal, context, previous_plan, previous_result, failure
    ):
...
class OpenAIPlanningProvider:
    ...
    def _validate_explicit_path(
        ...
        if (
            requested_path not in affected_files
            or requested_path not in operation_paths
        ):
            raise PlanningProviderError(
...
class OpenAICompatiblePlanningProvider(OpenAIPlanningProvider):
    """Planning adapter for OpenAI-compatible providers."""
    ...
    def __init__(
        ...
        *,
        ...
        compatible_provider = provider or OpenAICompatibleProvider(
...
class OllamaPlanningProvider:
    ...
    def generate_plan(self, goal: str, context: ProjectContext) -> Mapping[str, Any]:
        ...
        try:
            ...
            raise PlanningProviderError(
                f"Ollama returned an invalid structured plan: {error}"
            ) from error
        if not isinstance(payload, Mapping):
            raise PlanningProviderError(
                "Ollama returned a JSON value that is not a plan object."
            )
        return payload
...
def create_planning_provider(name: str) -> PlanningProvider:
    ...
    if provider_name == "deterministic":
        if separator:
            raise PlanValidationError(
                "The deterministic provider does not accept a model suffix."
            )
    ...
    if has_provider(provider_name):
        spec = get_provider_spec(provider_name)

        if spec.api_style != "chat_completions":
            raise PlanValidationError(
                f"Unsupported planning provider API style: {spec.api_style}"
            )

        return OpenAICompatiblePlanningProvider(
            model=spec.resolve_model(model or None),
